...
- **Grader-robustness + answer scope-discipline (stricter judge).** Re-run against a second, stricter grader (gpt-5.4) on the memory-off, link-neutralized 219-case set (stale PR/CI links removed from the eval questions so answers are graded on the technical question, not the PR's current merge state), the hybrid KB+graph configuration scores **63.5 %** vs **60.3 %** for the memory-off KB-only baseline (**+3.2 pp**), so the graph advantage holds under both graders. Adding an answer **scope-discipline** instruction — lead with the single direct verdict and stop graph-backbone (troubleshooting / process) answers from expanding into adjacent mechanisms, extra "blockers", or step-by-step diagnosis the user did not ask about — lifts the configuration to **66.2 %** (3-run mean; **+5.9 pp** over KB-only). This lever is deliberately orthogonal to *which* conclusion the agent reaches, so it trims the over-expansion losses without flipping the cases where graph correctly overrides KB, and answer length is essentially unchanged (the gain is from front-loading the correct verdict, not from shortening).
