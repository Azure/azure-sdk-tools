...
def grep_children_namespaces(toc_location):
    if toc_location.endswith('.yml'):
        with open(toc_location, "r") as toc_yml:
            target_autogenerated_toc = yaml.safe_load(toc_yml)
        return [
            top_level_namespace["name"] for top_level_namespace in target_autogenerated_toc
        ] + ["*"]
    elif toc_location.endswith('.xml'):
        tree = xml.parse(toc_location)
        root = tree.getroot()
        assemblies = root.findall('Assemblies')[0].getchildren()

        return [
            element.get('Name') for element in assemblies
        ] + ["*"]
    else:
        raise Exception("Unable to parse the known frameworks from {}".format(toc_location))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not os.path.exists(args.namespaces):
            raise FileNotFoundError

    except Exception as f:
        print(
            "Execution requires that both the known namespaces and target reference yml be defined."
        )

    present_in_target = grep_children_namespaces(args.namespaces)
