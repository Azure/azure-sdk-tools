...
describe("github-api module", () => {
  describe("parseGitHubPrUrl", () => {
    test("parses valid GitHub PR URL", () => {
      const result = parseGitHubPrUrl(
        "https://github.com/Azure/azure-sdk-for-net/pull/12345",
      );
    ...
    test("parses URL with trailing content", () => {
      const result = parseGitHubPrUrl(
        "https://github.com/Azure/azure-sdk-for-python/pull/99/files",
      );
      expect(result).toEqual({
        owner: "Azure",
    ...
    test("returns null for non-PR GitHub URLs", () => {
      expect(
        parseGitHubPrUrl("https://github.com/Azure/azure-sdk-for-net"),
      ).toBeNull();
      expect(
        parseGitHubPrUrl(
          "https://github.com/Azure/azure-sdk-for-net/issues/123",
        ),
      ).toBeNull();
    });
    ...
    test("returns null for non-GitHub URLs", () => {
      expect(
        parseGitHubPrUrl("https://dev.azure.com/org/project/pull/123"),
      ).toBeNull();
      expect(
        parseGitHubPrUrl("https://gitlab.com/org/repo/pull/123"),
      ).toBeNull();
    });

    test("handles complex repo names", () => {
      const result = parseGitHubPrUrl(
        "https://github.com/Azure/azure-rest-api-specs/pull/7890",
      );
      expect(result).toEqual({
        owner: "Azure",
  ...
  describe("throttledMap", () => {
    test("processes all items with concurrency control", async () => {
      ...
      const results = await throttledMap(items, async (x) => x * 2, {
        concurrency: 2,
        delayMs: 0,
      });
    ...
    test("handles empty array", async () => {
      const results = await throttledMap([], async (x) => x, {
        concurrency: 5,
        delayMs: 0,
      });
