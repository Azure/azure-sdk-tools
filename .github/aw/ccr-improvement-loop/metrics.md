# CCR Improvement Loop — metrics reference

This is the detailed, defensible definition of every metric the loop reports: for
each number, **what it measures, why the underlying signal is valid, its known
limits, and how to read it honestly.** The [`README`](README.md) has the quick
"what/how"; the engineering decisions behind the design are in
[`decisions.md`](decisions.md).

The guiding principle is **subtractive**: better three numbers we can defend than a
dozen that look rigorous but rest on weak proxies. An earlier iteration measured CCR
with structural shortcuts — line-range overlap, "a later commit touched the file",
regex sentiment on replies. Each was noisy, and no downstream schema rigor fixes a
bad proxy. Those are gone (see
[Rejected metrics](decisions.md#d1--subtractive-metric-set-defend-a-few-not-display-many)).
What remains comes from a **deterministic fact** or an **explicit LLM judgment made
from the actual evidence**, never from co-location or keyword guessing.
...
- **Backfill (manual):** pass explicit `window_start` / `window_end` to measure
  any historical window — see [Onboarding](README.md#onboarding).
