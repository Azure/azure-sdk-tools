...
- Added `detect-breaking-change` CLI command and `azsdk_package_detect_breaking_change` MCP tool to detect SDK breaking changes for a pacakge. Accepts the sdk package via `--package-path`. Optional parameters: `--tsp-config-path`, `--changes-only`.