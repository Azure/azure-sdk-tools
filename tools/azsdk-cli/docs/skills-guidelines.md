...
Create `tests/{skill-name}/prompts.json` for each skill:

```json
{
  "skillName": "your-skill-name",
  "shouldTrigger": ["Prompt that should match this skill"],
  "shouldNotTrigger": ["Prompt that should NOT match"]
}
```

For external skills, add a `source` field pointing to the SKILL.md:
```json
{
  "source": {
...
- **shouldTrigger**: Include diverse variations (direct, indirect, vague)
- **shouldNotTrigger**: Include related but different tasks

### Running Tests

```bash
dotnet test --filter "Category=skills" # From tools/azsdk-cli/Azure.Sdk.Tools.Cli.Evaluations
```
