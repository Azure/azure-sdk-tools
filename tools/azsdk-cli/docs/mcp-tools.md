...
This document provides a comprehensive list of all MCP (Model Context Protocol) tools supported by the Azure SDK MCP server version 0.5.9.
...
| azsdk_get_failed_test_cases | azsdk pkg test results | Get titles of failed test cases from a TRX file |
...
| azsdk_release_sdk | azsdk pkg release | Releases the specified SDK package for a language. This includes checking if the package is ready for release and triggering the release pipeline. This tool calls CheckPackageReleaseReadiness |
