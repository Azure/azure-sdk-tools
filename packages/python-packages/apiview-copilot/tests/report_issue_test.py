...
class TestHandleReportIssueRequestEndToEnd:
    ...
    @patch("src._report_issue.create_issue")
    @patch("src._report_issue.run_prompt")
    def test_copilot_issue_uses_avc_prefix_and_label(
        self, mock_run_prompt, mock_create_issue, _mock_github_manager
    ):
...
class TestLanguageLabels:
    def test_canonical_languages_present(self):
        # Must mirror OpenParserIssueWorkflow.LANGUAGE_LABELS so issues
        # raised manually here match those raised via @mention.
