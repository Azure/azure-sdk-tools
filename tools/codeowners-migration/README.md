...
> **Status:** the `convert` and `verify` verbs described here are implemented and tested.
>
> The YAML ownership model they target — `.github/owners.config.yaml`, `sdk/*/owners.yaml`, the
> `owners` JSON schemas, and the `azsdk config codeowners generate --repo-root` renderer — lands in a
> follow-up pull request. Until that merges, this README's references to the ownership spec, to
> `generate`, and to rule codes such as `CFG-DUP-001` describe the target design rather than what is
> currently on `main`, and the end-to-end workflow below cannot be run start to finish.
>
> This tool ships first because it depends only on `Azure.Sdk.Tools.CodeownersUtils`, so it builds
> and tests independently of that work.
