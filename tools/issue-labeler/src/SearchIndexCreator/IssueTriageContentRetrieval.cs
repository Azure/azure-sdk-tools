    ...
    public class IssueTriageContentRetrieval
    {
        ...
        private async Task<List<IssueTriageContent>> RetrieveAllIssuesAsync(string repoOwner)
        {
            ...
            foreach (var issue in issues)
            {
                ...
                {
                    Id = $"{repoOwner}/{_repo}/{issue.Number.ToString()}/Issue",
                    Title = issue.Title,
                    Body = $"{issue.Body}",
                ...
                foreach (var comment in issue_comments)
                {
                    ...
                    {
                        Id = $"{repoOwner}/{_repo}/{issue.Number.ToString()}/{comment.Id.ToString()}",
                        Title = issue.Title,
                        Body = $"{comment.Body}",
                        Service = service.Name,
                        Category = category.Name,
        ...
        private (Label service, Label category) GetServiceAndCategoryLabels(IReadOnlyList<Label> labels)
        {
