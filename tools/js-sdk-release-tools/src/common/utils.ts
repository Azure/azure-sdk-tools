...
export async function resolveOptions(
  ...
  const [{ config, ...options }, diagnostics] = await compiler.resolveCompilerOptions(compiler.NodeHost, {
    cwd: process.cwd(),
 ...
 */
export async function cleanUpDirectory(directory: string, entriesToPreserve: string[] = []): Promise<void> {
 ...
 * @returns Promise that resolves when cleanup is complete
 */
export async function cleanUpPackageDirectory(packageDirectory: string, runMode: RunMode): Promise<void> {
  // Check if directory exists first
