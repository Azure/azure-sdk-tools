...
- **Graph must not bloat the answer.** The graph tool returns more references than the KB alone — but more references must NOT mean a longer answer. The graph earns its place only by (a) supplying a specific fact the KB is missing, or (b) correcting a KB statement that is wrong. If it does neither, ignore it. **Never let graph references add length, extra options, background, restatement, or hedging to an answer the KB already resolves.** Keep every answer as tight as it would be from the KB alone — a graph-grounded answer to a given question should be no longer than a focused KB-only answer to the same question. When the KB already gives a crisp, correct answer, do not expand or soften it just because the graph surfaced related material.
