...
export function ghJsonSync<T>(args: string[]): T {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    throw new Error(`failed to parse JSON from gh ${args.join(" ")}: ${msg}`);
  }
}
...
export function ghApiJsonAsync<T>(endpoint: string): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        reject(new Error(`gh api ${endpoint} failed: ${stderr.trim()}`));
        return;
      }
      try {
  ...
  async function runWorker(): Promise<void> {
    for (;;) {
      ...
      try {
        ...
        active--;
        completed++;
        onProgress?.({ type: "finish", item, completed, active, total });
      }
    }
  }
