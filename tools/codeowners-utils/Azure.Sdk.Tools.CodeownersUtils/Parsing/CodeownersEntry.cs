...
namespace Azure.Sdk.Tools.CodeownersUtils.Parsing
{
    ...
    public class CodeownersEntry
    {
        ...
        /// <summary>
        /// Formats a codeowners entry in the specific format with PR labels, service labels, and owners.
        /// Supports both PRLabel/path entries and ServiceLabel/ServiceOwners entries.
        /// </summary>
        /// <param name="includePRLabelEntry">Whether to include the PRLabel and path entry</param>
        /// <param name="includeServiceLabelEntry">Whether to include the ServiceLabel and ServiceOwners entry</param>
        /// <returns>Formatted codeowners entry string</returns>
        public string FormatCodeownersEntry(bool includePRLabelEntry = true, bool includeServiceLabelEntry = true)
        {
            var result = new StringBuilder();
            var entries = new List<string>();

            // Generate PRLabel and path entry
            if (includePRLabelEntry && !string.IsNullOrWhiteSpace(PathExpression))
            {
                var prLabelEntry = new StringBuilder();
                
                // Add PRLabel comment if PRLabels exist
                if (PRLabels.Any())
                {
                    var formattedPRLabels = PRLabels.Select(label => label.StartsWith("%") ? label : $"%{label}");
                    prLabelEntry.AppendLine($"# PRLabel: {string.Join(" ", formattedPRLabels)}");
                }
                
                // Add path and source owners
                if (SourceOwners.Any())
                {
                    var formattedSourceOwners = SourceOwners.Select(owner => owner.StartsWith("@") ? owner : $"@{owner}");
                    var pathLine = $"{PathExpression}".PadRight(67) + $"{string.Join(" ", formattedSourceOwners)}";
                    prLabelEntry.AppendLine(pathLine);
                }

                if (prLabelEntry.Length > 0)
                {
                    entries.Add(prLabelEntry.ToString().TrimEnd());
                }
            }

            // Generate ServiceLabel and ServiceOwners entry
            if (includeServiceLabelEntry && (ServiceLabels.Any() || ServiceOwners.Any()))
            {
                var serviceLabelEntry = new StringBuilder();
                
                // Add ServiceLabel comment if ServiceLabels exist
                if (ServiceLabels.Any())
                {
                    var formattedServiceLabels = ServiceLabels.Select(label => label.StartsWith("%") ? label : $"%{label}");
                    serviceLabelEntry.AppendLine($"# ServiceLabel: {string.Join(" ", formattedServiceLabels)}");
                }

                // Add AzureSdkOwners if they exist
                if (AzureSdkOwners.Any())
                {
                    var formattedAzureSdkOwners = AzureSdkOwners.Select(owner => owner.StartsWith("@") ? owner : $"@{owner}");
                    serviceLabelEntry.AppendLine($"# AzureSdkOwners:{new string(' ', 50)}{string.Join(" ", formattedAzureSdkOwners)}");
                }
                
                // Add ServiceOwners if they exist
                if (ServiceOwners.Any())
                {
                    var formattedServiceOwners = ServiceOwners.Select(owner => owner.StartsWith("@") ? owner : $"@{owner}");
                    serviceLabelEntry.AppendLine($"# ServiceOwners:{new string(' ', 51)}{string.Join(" ", formattedServiceOwners)}");
                }


                if (serviceLabelEntry.Length > 0)
                {
                    entries.Add(serviceLabelEntry.ToString().TrimEnd());
                }
            }

            // Combine entries with a blank line between them if both exist
