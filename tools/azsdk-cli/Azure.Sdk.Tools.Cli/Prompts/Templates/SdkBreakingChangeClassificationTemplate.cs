...
namespace Azure.Sdk.Tools.Cli.Prompts.Templates
{
    public class SdkBreakingChangeClassificationTemplate : BasePromptTemplate
    {
        ...
        protected override string BuildSystemRole()
        {
            ...
            Your task is to {Description} by determining the breaking changes and their categories. You MUST review each SDK breaking change item in `sdk changes ### Breaking Changes` exactly once. After reviewing all items, merge related items into one classified SDK breaking change only when they clearly share the same root cause, then provide the matched resolution when one exists.
            Each classified out SDK breaking change must be output as separate one. If an SDK breaking change item in `sdk changes ### Breaking Changes` does not match any provided SDK breaking change pattern, keep the original breaking change text unchanged.
