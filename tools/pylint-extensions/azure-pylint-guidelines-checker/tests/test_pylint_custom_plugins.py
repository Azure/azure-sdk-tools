...
class TestCheckMissingDependency(pylint.testutils.CheckerTestCase):
    ...
    def test_violation_missing_dep_poetry_pyproject(self):
        """Imports of known 3rd party packages not in pyproject.toml Poetry deps should be flagged."""
        filepath = os.path.join(
            TEST_FOLDER,
            "test_files",
            "missing_dep_poetry_violation_pkg",
            "violation.py",
        )
        node = astroid.MANAGER.ast_from_file(filepath)
