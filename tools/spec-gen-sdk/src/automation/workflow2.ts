...
import { runSdkAutoCustomScript, setSdkAutoStatus } from '../utils/runScript2';
import {
  deleteTmpJsonFile,
  readTmpJsonFile,
...
import { getPackageData, PackageData } from '../types/PackageData2';
import { workflowPkgMain } from './workflowPackage2';
...
import { parseYamlContent, removeDuplicatesFromRelatedFiles } from '../utils/utils2';
import { SDKSuppressionContentList } from '../utils/handleSuppressionLines';
import { SdkAutoContext_New } from './entrypoint2';
...
export type WorkflowContext = SdkAutoContext_New & {
...
export const workflowInit = async (context: SdkAutoContext_New): Promise<WorkflowContext> => {
  const messages = [];
...
export const workflowValidateSdkConfigForSpecPr = async (context: SdkAutoContext_New) => {
...
export const workflowValidateSdkConfig = async (context: SdkAutoContext_New) => {
  const specConfigContent = workflowInitGetSpecConfig(context);
  const specConfig = getSpecConfig(specConfigContent, context.config.specRepo);
...
export const workflowInitGetSpecConfig = (context: SdkAutoContext_New) => {
...
export const workflowInitGetDefaultBranch = async (context: SdkAutoContext_New, repo: RepoKey) => {
...
const workflowInitConfig = async (context: SdkAutoContext_New) => {
...
const workflowDetectChangedSpec = async (
  context: Pick<WorkflowContext, 'specRepo' | 'logger' | 'specFolder' | keyof SdkAutoContext_New>
) => {
