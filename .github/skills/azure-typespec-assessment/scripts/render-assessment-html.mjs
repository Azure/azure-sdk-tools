...
function renderRestContractCards(findings = []) {
  return restContractCards(findings)
    .map((card) => {
      ...
      return `<details class="finding" id="${restContractCardId(card)}"><summary><strong>${escapeHtml(card.identity)}</strong><span class="tag rest-breaking-tag">REST contract</span><span class="finding-summary-meta">${card.findings.length} contract changes — ${card.operations.length} affected REST operations</span></summary>
