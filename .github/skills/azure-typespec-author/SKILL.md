...
2. **Collapse unreleased previews.** Treat the latest existing **preview** version as unreleased (collapse it) unless the user explicitly says that preview already shipped. If the latest existing version is an **unreleased preview**, collapse it into the new version: remove it from the `Versions` enum, rename its `examples/<old-preview>/` folder to `examples/<new-version>/` (no `examples/<old-preview>/` may remain), and rebase **every** decorator that referenced the preview (`@added`, `@removed`, `@renamedFrom`, `@typeChangedFrom`) onto the new version — keep the decorator, just change its version argument. **Do NOT drop decorators to plain baseline when an older retained released version exists**: a preview-only `@added` property that you carry keeps `@added(Versions.<new-version>)` (do not remove `@added`), and a preview `@removed`/`@renamedFrom`/`@typeChangedFrom` is retargeted to the new version so backward compatibility with the released version is preserved. For a feature that is **excluded**, delete it outright — remove the property/model **and all of its decorators** (no `@removed`). Making a carried feature plain baseline is correct only when there is no older released version at all. If the latest existing version is a **released stable**, add the new version incrementally and keep the stable (do not collapse). This collapse behavior applies even when the fetched versioning doc shows an incremental (keep-the-preview) approach.
