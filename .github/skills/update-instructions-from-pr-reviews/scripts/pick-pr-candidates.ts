...
function parseCli(argv: string[]): Options {
  ...
  if (parsed.values.exclude) {
    for (const n of parseNumbers(parsed.values.exclude)) excludeNumbers.add(n);
  }
  if (parsed.values["exclude-file"]) {
    for (const n of readExcludeFile(parsed.values["exclude-file"])) excludeNumbers.add(n);
  }

  return {
    glob,
    exclude: excludeNumbers,
    minInline: parsePositiveInt(parsed.values["min-inline"], "--min-inline"),
...
export function selectCandidates(all: Candidate[], opts: Options): Candidate[] {
  return all
    ...
    .sort((a, b) => {
      if (b.humanInline !== a.humanInline) return b.humanInline - a.humanInline;
      if (b.humanTotal !== a.humanTotal) return b.humanTotal - a.humanTotal;
      return b.pr - a.pr;
    })
    .slice(0, opts.limit);
}
