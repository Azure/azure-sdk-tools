...
public class CustomizedCodeUpdateToolAutoTests
{
    ...
    #region HasCustomizationFiles Tests

    [Test]
    public void HasCustomizationFiles_Java_WithCustomizationDirectory_ReturnsTrue()
    {
        var gitHelper = new Mock<IGitHelper>();
        var tsp = new MockTspHelper();
        var specGenSdkConfigHelper = new Mock<ISpecGenSdkConfigHelper>();
        ...
        var method = typeof(CustomizedCodeUpdateTool).GetMethod("HasCustomizationFiles", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var result = (bool)method!.Invoke(tool, new object[] { pkg, svc })!;

        Assert.That(result, Is.True, "Should detect Java customization directory");
    }

    [Test]
    public void HasCustomizationFiles_Java_WithCustomizationClass_ReturnsTrue()
    {
        var gitHelper = new Mock<IGitHelper>();
        var tsp = new MockTspHelper();
        var specGenSdkConfigHelper = new Mock<ISpecGenSdkConfigHelper>();
        ...
        var pkg = CreateTempPackageDir();
        var srcDir = Path.Combine(pkg, "src", "main", "java");
        Directory.CreateDirectory(srcDir);
        File.WriteAllText(Path.Combine(srcDir, "FooCustomization.java"), "public class FooCustomization {}");

        var method = typeof(CustomizedCodeUpdateTool).GetMethod("HasCustomizationFiles", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var result = (bool)method!.Invoke(tool, new object[] { pkg, svc })!;

        Assert.That(result, Is.True, "Should detect Java customization class file");
    }

    [Test]
    public void HasCustomizationFiles_Python_WithPatchFile_ReturnsTrue()
    {
        var gitHelper = new Mock<IGitHelper>();
        var tsp = new MockTspHelper();
        var specGenSdkConfigHelper = new Mock<ISpecGenSdkConfigHelper>();
        ...
        var pkg = CreateTempPackageDir();
        var patchFile = Path.Combine(pkg, "azure", "foo", "_patch.py");
        Directory.CreateDirectory(Path.GetDirectoryName(patchFile)!);
        File.WriteAllText(patchFile, "# Patch file");

        var method = typeof(CustomizedCodeUpdateTool).GetMethod("HasCustomizationFiles", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var result = (bool)method!.Invoke(tool, new object[] { pkg, svc })!;

        Assert.That(result, Is.True, "Should detect Python _patch.py file");
    }

    [Test]
    public void HasCustomizationFiles_DotNet_WithPartialClass_ReturnsTrue()
    {
        var gitHelper = new Mock<IGitHelper>();
        var tsp = new MockTspHelper();
        var specGenSdkConfigHelper = new Mock<ISpecGenSdkConfigHelper>();
        var svc = new MockDotNetLanguageService();
        var tool = new CustomizedCodeUpdateTool(new NullLogger<CustomizedCodeUpdateTool>(), [svc], gitHelper.Object, tsp, specGenSdkConfigHelper.Object);
        
        var pkg = CreateTempPackageDir();
        var srcDir = Path.Combine(pkg, "src");
        Directory.CreateDirectory(srcDir);
        File.WriteAllText(Path.Combine(srcDir, "CustomClient.cs"), 
            "namespace Azure.Foo { public partial class FooClient { } }");

        var method = typeof(CustomizedCodeUpdateTool).GetMethod("HasCustomizationFiles", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var result = (bool)method!.Invoke(tool, new object[] { pkg, svc })!;

        Assert.That(result, Is.True, "Should detect .NET partial class");
    }

    [Test]
    public void HasCustomizationFiles_NoCustomizations_ReturnsFalse()
    {
        var gitHelper = new Mock<IGitHelper>();
        var tsp = new MockTspHelper();
        var specGenSdkConfigHelper = new Mock<ISpecGenSdkConfigHelper>();
        ...
        Assert.That(result, Is.False, "Should return false when no customizations exist");
    }

    private class MockDotNetLanguageService : LanguageService
    {
