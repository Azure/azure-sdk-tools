...
function ProcessPackage($packageName) {
  ...
  if ($FindArtifactForApiReviewFn -and (Test-Path "Function:$FindArtifactForApiReviewFn")) {
    ...
    Write-Host "The function for 'FindArtifactForApiReviewFn' was not found.`nMake sure it is present in eng/scripts/Language-Settings.ps1 and referenced in eng/common/scripts/common.ps1.`nSee https://github.com/Azure/azure-sdk-tools/blob/main/doc/common/common_engsys.md#code-structure"
    return 1
  }
  ...
  if ($packages) {
    foreach ($pkgPath in $packages.Values) {
      ...
      if ( ($SourceBranch -eq $DefaultBranch) -or (-not $version.IsPrerelease) -or $MarkPackageAsShipped) {
        ...
        $apiStatus = [PSCustomObject]@{ IsApproved = $false; Details = "" }
        $pkgNameStatus = [PSCustomObject]@{ IsApproved = $false; Details = "" }
        Process-ReviewStatusCode $respCode $packageName $apiStatus $pkgNameStatus

        # Management plane namespace-only gating for Java, JS, Python, Go
        $isMgmtNamespaceOnly = ($pkgInfo.SdkType -eq 'mgmt' -and ($LanguageShort -in @('java','js','python','py','go','golang')))
...
foreach ($pkg in $responses.keys) {
  if ($responses[$pkg] -eq 1) {
    Write-Host "API changes or required namespace approval are not satisfied for $($pkg)"
    $exitCode = 1
  }
}
exit $exitCode
