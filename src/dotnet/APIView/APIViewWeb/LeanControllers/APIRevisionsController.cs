            ...
            if (filterAndSortParams.APIRevisionIds != null && filterAndSortParams.APIRevisionIds.Any())
            {
                foreach (var apiRevisionId in filterAndSortParams.APIRevisionIds)
                {
                    ...
                    if (apiRevision != null && apiRevision.CopilotReviewInProgress)
                    {
                        ...
                        {
                            var hasAutoGeneratedComments = string.Equals(pollResponse.Status, "Success", StringComparison.OrdinalIgnoreCase) ? true : (bool?)null;
                            await _apiRevisionsManager.UpdateAPIRevisionCopilotDetailsAsync(apiRevision, copilotReviewInProgress: false, hasAutoGeneratedComments: hasAutoGeneratedComments);
