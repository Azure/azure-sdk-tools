  ...
  describe("enums", () => {
    ...
    describe("enum variations end-to-end", () => {
      ...
      it("generates simple enum with members correctly", () => {
        ...
        const memberTokens =
          enumLine?.Children?.flatMap((c) => c.Tokens.map((t) => t.Value).join("")) ?? [];
      ...
      it("generates nested enum in namespace correctly", () => {
        ...
        expect(nestedEnum?.Children?.[0].LineId).toBe(
          "@azure/enum-test!MyNamespace.NestedEnum.First:member",
        );
        expect(nestedEnum?.Children?.[1].LineId).toBe(
          "@azure/enum-test!MyNamespace.NestedEnum.Second:member",
        );
      });
      ...
      it("generates deeply nested enum in namespace hierarchy", () => {
        ...
        expect(deepEnum?.Children?.[0].LineId).toBe(
          "@azure/enum-test!MyNamespace.Nested.DeeplyNestedEnum.DeepValue:member",
        );
      });
