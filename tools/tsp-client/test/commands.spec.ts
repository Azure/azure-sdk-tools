...
describe.sequential("Verify commands", () => {
  ...
  it("Generate config files strips workspace prefix as fallback for semver workspace versions", async () => {
    const tmpDir = joinPaths(cwd(), ".tmp-test-workspace-semver");
    try {
      await mkdir(tmpDir, { recursive: true });
      const emitterPackageJsonPath = joinPaths(tmpDir, "emitter-package-workspace-semver.json");

      const args = {
        "package-json": joinPaths(cwd(), "test", "examples", "package-workspace-with-semver.json"),
        "emitter-package-json-path": emitterPackageJsonPath,
      };

      // Lock file generation will fail for the fake package, but the
      // emitter-package.json is written before that step — verify its content.
      try {
        await generateConfigFilesCommand(args);
      } catch {
        // Expected: npm install fails for non-existent test package
      }

      assert.isTrue(
        await doesFileExist(emitterPackageJsonPath),
        "emitter-package.json should be created before lock file step",
      );
      const emitterJson = JSON.parse(await readFile(emitterPackageJsonPath, "utf8"));

      // The emitter should be added as a dependency
      assert.equal(emitterJson["dependencies"]["@test/fake-emitter-workspace-semver"], "0.0.1");

      // Workspace versions with semver should be stripped to just the semver part
      // npm view will fail for the non-existent package, so fallback stripping applies
      assert.equal(emitterJson["devDependencies"]["@typespec/compiler"], "~1.6.0");
      assert.equal(emitterJson["devDependencies"]["@azure-tools/typespec-azure-core"], "~0.62.0");
    } finally {
      if (await doesFileExist(tmpDir)) {
        await rm(tmpDir, { recursive: true });
      }
  ...
  it("Generate config files skips bare workspace versions when npm view fails", async () => {
    const tmpDir = joinPaths(cwd(), ".tmp-test-workspace-bare");
    try {
      await mkdir(tmpDir, { recursive: true });
      const emitterPackageJsonPath = joinPaths(tmpDir, "emitter-package-workspace-bare.json");

      const args = {
        "package-json": joinPaths(cwd(), "test", "examples", "package-workspace.json"),
        "emitter-package-json-path": emitterPackageJsonPath,
      };
      ...
      // The emitter should be added as a dependency
      assert.equal(emitterJson["dependencies"]["@test/fake-emitter-workspace"], "0.0.1");

      // Bare workspace:~ versions cannot be resolved (npm view fails for non-existent package)
      // so they should be skipped — devDependencies should be empty or not set
      assert.isTrue(
        !emitterJson["devDependencies"] || Object.keys(emitterJson["devDependencies"]).length === 0,
        "devDependencies should be empty when bare workspace versions cannot be resolved",
      );
    } finally {
      if (await doesFileExist(tmpDir)) {
        await rm(tmpDir, { recursive: true });
