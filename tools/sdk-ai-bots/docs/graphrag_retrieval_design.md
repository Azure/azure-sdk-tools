...
- **Grader-robustness (stricter judge).** Re-run against a second, stricter grader (gpt-5.4) on the same memory-off datasets, the hybrid KB+graph configuration scores **64.1 %** vs **60.5 %** for the memory-off KB-only baseline (**+3.6 pp**) — driven by general (+19 pp), apispec (+14 pp), and python (+7 pp), tying typespec. The absolute rates fall under the harsher judge, but the graph advantage holds under both graders, so the benefit is not an artifact of a single grader.
