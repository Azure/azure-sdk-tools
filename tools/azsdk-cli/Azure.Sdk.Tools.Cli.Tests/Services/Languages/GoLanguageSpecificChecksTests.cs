...
namespace Azure.Sdk.Tools.Cli.Tests.Services
{
    internal class GoLanguageSpecificChecksTests
    {
        ...
        public async Task TestGoLanguageSpecificChecksBasic()
        {
            ...
            var resp = await LangService.AnalyzeDependencies(GoPackageDir, false, CancellationToken.None);
            Assert.That(resp.ExitCode, Is.EqualTo(0));
            ...
            resp = await LangService.FormatCode(GoPackageDir, false, CancellationToken.None);
            Assert.That(File.ReadAllText(Path.Join(GoPackageDir, "main.go")), Does.Not.Contain("azservicebus"));

            resp = await LangService.BuildProject(GoPackageDir, CancellationToken.None);
            Assert.That(resp.ExitCode, Is.EqualTo(0));

            resp = await LangService.LintCode(GoPackageDir, false, CancellationToken.None);
            Assert.That(resp.ExitCode, Is.EqualTo(0));
        }
        ...
        public async Task TestGoLanguageSpecificChecksCompileErrors()
        {
            ...
            var resp = await LangService.BuildProject(GoPackageDir, CancellationToken.None);
            Assert.Multiple(() =>
            {
        ...
        public async Task TestGoLanguageSpecificChecksLintErrors()
        {
            ...
            var resp = await LangService.LintCode(GoPackageDir, false, CancellationToken.None);
            Assert.Multiple(() =>
            {
                Assert.That(resp.ExitCode, Is.EqualTo(1));
