...
test('HLC -> Modular: Rename', async () => {
  ...
  const changelog = await extractExportAndGenerateChangelog(oldViewPath, newViewPath, SDKType.HighLevelClient, SDKType.ModularClient);
  ...
  expect(changelog.addedOperation[0]).toBe('Added operation DataProductsCatalogsOperations.listByResourceGroup_NEW');
  expect(changelog.removedOperation[0]).toBe('Removed operation DataProductsCatalogs.listByResourceGroup_NEW');
})
...
test('HLC -> HLC: Change Op', async () => {
  ...
  const changelog = await extractExportAndGenerateChangelog(oldViewPath, newViewPath, SDKType.HighLevelClient, SDKType.HighLevelClient);
  ...
  expect(changelog.addedOperation[0]).toBe('Added operation DataProductsCatalogs.get_NEW');
  expect(changelog.removedOperation[0]).toBe('Removed operation DataProductsCatalogs.get_NEW');
})
...
test('Modular -> Modular: Change Op', async () => {
  ...
  const changelog = await extractExportAndGenerateChangelog(oldViewPath, newViewPath, SDKType.ModularClient, SDKType.ModularClient);
  ...
  expect(changelog.addedOperation[0]).toBe('Added operation DataProductsCatalogsOperations.listByResourceGroup_NEW');
  expect(changelog.removedOperation[0]).toBe('Removed operation DataProductsCatalogsOperations.listByResourceGroup_NEW');
})