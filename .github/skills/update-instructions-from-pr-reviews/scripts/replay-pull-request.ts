...
export function parseCli(argv: string[]): Options {
  ...
  return {
    ...
    concurrency: parsePositiveInt(parsed.values.concurrency, "--concurrency"),
...
export function buildRounds(
  ...
  for (const commit of commits) {
    ...
    roundIndex++;
    const inlineFiles = [
      ...new Set(commitInline.map((c) => c.path).filter(Boolean) as string[]),
    ];
...
export function extractPrUrlFromSetupOutput(output: string): string {
  const prMatch = /^PR created: (https:\/\/github\.com\/[^\s]+)$/m.exec(output);
  return prMatch?.[1] ?? "";
}
...
async function processSinglePr(
  ...
  log.info(
    `base=${baseSha.slice(0, 12)} head=${pr.head.sha.slice(0, 12)}`,
  );
  ...
  log.info(
    `${rounds.length} round(s) with human feedback`,
  );
  ...
  const targetRounds =
    opts.round != null ? rounds.filter((r) => r.index === opts.round) : rounds;
...
async function main(): Promise<void> {
  ...
  const perPr = await runWithConcurrency(
    ...
    async (prNumber) => {
      const branchPrefix =
        targetPrs.length > 1 ? `${opts.branch}-${prNumber}` : opts.branch;
  ...
  log.info(
    `wave complete in ${formatElapsed(Date.now() - waveStartMs)}`,
  );
  ...
  if (!opts.dryRun) {
    ...
    if (targetPrs.length === 1) {
      process.stdout.write(
        "\n" +
          JSON.stringify({ rounds: nonEmpty[0]?.rounds ?? [] }, null, 2) +
          "\n",
      );
    } else {
      process.stdout.write(
