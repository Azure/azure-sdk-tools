...
namespace APIViewWeb.LeanControllers
{
    public class CommentsController : BaseApiController
    {
        ...
        {
            var autGeneratedComments = await _commentsManager.GetAPIRevisionCommentsAsync(apiRevisionId: apiRevisionId, createdBy: "azure-sdk");
            foreach (var comment in autGeneratedComments)
            {
                await _commentsManager.SoftDeleteCommentAsync(User, comment.ReviewId, comment.Id);
            }
            return Ok();
        }
    }
