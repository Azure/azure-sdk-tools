...
| azsdk_update_api_spec_pull_request_in_release_plan | `azsdk release-plan update-spec-pr` | Update TypeSpec pull request URL in a release plan using work item id or release plan id. Pins the merged spec commit SHA for subsequent SDK generation; an unmerged PR clears any previous pin until it merges. Use the release plan for the intended API version. |
