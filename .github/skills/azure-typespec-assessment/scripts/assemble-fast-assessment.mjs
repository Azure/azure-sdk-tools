...
function validateFinding(item, label, evidence, { compliance = false } = {}) {
  ...
  if (compliance) {
    assert(
      item.sourceChangeIds.length > 0,
      `${label} must link to an exact changed TypeSpec source location.`,
    );
    assert(
      item.sourceChangeIds.every(
        (id) => evidence.sourceChanges.get(id)?.sourceLink,
      ),
      `${label} must provide a TypeSpec source link for every referenced change.`,
    );
...
function sourceLocation(snippet) {
...
function renderFinding(
  ...
  const snippets = selectedSnippets
    .map((snippet) => {
      const location = sourceLocation(snippet);
      const diffLines =
        snippet.diffLines ??
...
<p class="sources"><strong>TypeSpec source:</strong> ${selectedSnippets.map(sourceLocationLink).join(", ")}</p>
...
</div></header><aside class="preview-notice"><div class="container"><h2>Preview Notice</h2><p>The TypeSpec Assessment Assistant is currently in preview. Its goal is to help service developers build confidence earlier in the TypeSpec authoring workflow by providing contextual analysis, risk identification, and guidance on potential downstream impacts.</p><p>This report is intended as a review reference and learning aid only. It does not replace official ARM API, Azure API Stewardship, Azure Breaking Change reviews, and should not be treated as authoritative review feedback or approval for specification changes. Official validation tools, generated artifacts, and reviewer feedback remain the source of truth for merge and release decisions.</p></div></aside><nav><div class="container"><a href="#rest">REST breaking changes</a><a href="#downstream">SDK/downstream changes</a><a href="#compliance">Azure compliance</a></div></nav><main class="container">${blockers}
