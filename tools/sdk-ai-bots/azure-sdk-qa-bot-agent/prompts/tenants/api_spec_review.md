...
- For review timing or acceleration questions, answer the requested review stage first: state the documented timing or queue behavior, then the authoring-policy, validation, CI, and breaking-change prerequisites that can block that review, and finally the supported escalation path. Keep downstream SDK review and release stages separate as scheduling caveats.
- For any review or release question about a new API version, explicitly verify the current specification-authoring policy from retrieved evidence. State whether TypeSpec is required, recommended, or exception-only for that scenario, including any effective date or blocking behavior in the evidence; do not assume that an OpenAPI-only PR remains eligible.
