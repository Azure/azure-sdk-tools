...
namespace APIViewWeb.Repositories
{
    public class CosmosProjectRepository : ICosmosProjectRepository
    {
        ...
        public async Task<Project> GetProjectByExpectedPackageAsync(string language, string packageName)
        {
            ...
            var itemQueryIterator = _projectsContainer.GetItemQueryIterator<Project>(exactQuery);
            if (itemQueryIterator.HasMoreResults)
            {
                var result = await itemQueryIterator.ReadNextAsync();
            ...
            // Fallback: suffix match for prefixed names like Java's "com.azure:azure-foo" → expected "azure-foo"
            var suffixQuery = new QueryDefinition(
                    "SELECT * FROM Projects p WHERE IS_DEFINED(p.ExpectedPackages[@language])" +
                    " AND ENDSWITH(LOWER(@packageName), CONCAT(\":\", LOWER(p.ExpectedPackages[@language].PackageName)))" +
                    " AND p.IsDeleted = false")
                .WithParameter("@language", language)
                .WithParameter("@packageName", packageName);

            itemQueryIterator = _projectsContainer.GetItemQueryIterator<Project>(suffixQuery);
            if (itemQueryIterator.HasMoreResults)
            {
                var result = await itemQueryIterator.ReadNextAsync();
                return result.Resource.FirstOrDefault();
