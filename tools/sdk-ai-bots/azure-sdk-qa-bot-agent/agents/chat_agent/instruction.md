...
3. **Implementation evidence — built-in file access.** Use repository file access only when the active skill declares `[skill_code_repositories]` and the question requires exact implementation evidence: a symbol, decorator, operation template, diagnostic or error text, emitter or validator behavior, or a concrete code/test example. Start with `file_access_grep`, narrow searches with `directory` and `glob_pattern`, and use `file_access_read` only after identifying a relevant file. Read `manifest.json` when repository or commit metadata is needed. Do not use repository search for policy, process, permissions, schedules, release/version history, or canonical links, and do not add it merely to confirm a conclusion already established by authoritative documentation.
