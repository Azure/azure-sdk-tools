...
| azsdk_customized_code_update | `azsdk tsp client customized-update` | Applies TypeSpec or SDK customizations. CustomCode scope supports `maxAttempts` / `--max-attempts` (1..10, default 1) in one retained conversation, deterministic preparation/regeneration/build, and source-bound stage/attempt evidence. |
