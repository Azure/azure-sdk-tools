...
export function typeToReviewTokens(type: Type): ReviewToken[] {
    ...
    if (typeof type === "string") {
        ...
        let result: ReviewToken[] = [{ Kind: TokenKind.TypeName, Value: type.resolved_path.name, HasSuffixSpace: false, NavigateToId: type.resolved_path.id.toString() }];
        if (type.resolved_path.args && "angle_bracketed" in type.resolved_path.args) {
            const args = type.resolved_path.args.angle_bracketed.args
                ...
                .map((arg) => typeToReviewTokens(arg.type)).flatMap((a) => a)
            if (args.length > 0) {
        ...
        return [
            ...
            ...type.dyn_trait.traits.flatMap((t, i) => [
                ...
                i < type.dyn_trait.traits.length - 1 ? { Kind: TokenKind.Punctuation, Value: "+ " } : { Kind: TokenKind.Text, Value: "" },
            ]),
            { Kind: TokenKind.Punctuation, Value: ")", HasSuffixSpace: false },
        ];
        ...
        return [
            ...
            ...type.tuple.flatMap((t, i) => [
                ...
                i < type.tuple.length - 1 ? { Kind: TokenKind.Punctuation, Value: "," } : { Kind: TokenKind.Text, Value: "", HasSuffixSpace: false },
            ]),
            { Kind: TokenKind.Punctuation, Value: ")", HasSuffixSpace: false },
        ];
            ...
            ...type.impl_trait.flatMap((b, i) => {
                if ("trait_bound" in b) {
                    return [
                        ...
                        i < type.impl_trait.length - 1 ? { Kind: TokenKind.Punctuation, Value: "+", HasSuffixSpace: false } : { Kind: TokenKind.Text, Value: "", HasSuffixSpace: false },
                    ];
                    ...
                    return [
                        ...b.use.flatMap((u, j) => [
                            ...
                            j < b.use.length - 1 ? { Kind: TokenKind.Punctuation, Value: "," } : { Kind: TokenKind.Text, Value: "", HasSuffixSpace: false },
                        ]),
                    ];
                }
            }),
        ...
        return [
            { Kind: TokenKind.Punctuation, Value: "*", HasSuffixSpace: false },
            { Kind: TokenKind.Keyword, Value: type.raw_pointer.is_mutable ? "mut" : "const", HasSuffixSpace: false },
        ...
        return [
            { Kind: TokenKind.Punctuation, Value: "&", HasSuffixSpace: false },
            { Kind: TokenKind.Keyword, Value: type.borrowed_ref.is_mutable ? "mut" : "", HasSuffixSpace: false },
        ...
        return [
            ...
            { Kind: TokenKind.TypeName, Value: type.qualified_path.trait ? type.qualified_path.trait.name + "::" : "", HasSuffixSpace: false },
