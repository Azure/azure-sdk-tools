...
namespace Azure.Sdk.Tools.Cli.Tools.GitHub
{
    ...
    ) : MCPNoCommandTool
    {
        ...
        public async Task<GetPullRequestResponse> GetPullRequestForCurrentBranch(string repoPath, CancellationToken ct)
        {
            try
            {
                ...
                {
                    return new GetPullRequestResponse { ResponseError = "Failed to get repo root path. Please make sure to provide a valid repository path." };
                }
                ...
                {
                    return new GetPullRequestResponse { ResponseError = "Failed to get repo details. Please make sure to provide a valid repository path and try again." };
                }
                ...
                if (pullRequest == null)
                {
                    return new GetPullRequestResponse { ResponseError = "No pull request found for the current branch." };
                }

                var prDetails = await BuildPullRequestDetailsAsync(repoOwner, repoName, pullRequest.Number, repoPath, ct);
            ...
            catch (Exception ex)
            {
                ...
                return new GetPullRequestResponse { ResponseError = $"Failed to find pull request for current branch, Error: {ex.Message}" };
            }
        }
        ...
        public async Task<CreatePullRequestResponse> CreatePullRequest(string title, string description, string repoPath, string targetBranch = "main", bool draft = true, CancellationToken ct = default)
        {
            List<string> results = [];
            try
            {
                ...
                if (!headBranchExists)
                {
                    return new CreatePullRequestResponse
                    {
                        Messages = [$"Head branch '{headBranch}' was not found on GitHub."],
                ...
                return new CreatePullRequestResponse
                {
                    PullRequestUrl = createResponse.Url,
                    Messages = results
                };
            }
            catch (Exception ex)
        ...
        public async Task<GetPullRequestResponse> GetPullRequest(int pullRequestNumber, string repoPath, CancellationToken ct)
        {
            try
            {
                var repoRootPath = await gitHelper.DiscoverRepoRootAsync(repoPath, ct);
                ...
                var prDetails = await BuildPullRequestDetailsAsync(repoOwner, repoName, pullRequestNumber, repoPath, ct);
            ...
            catch (Exception ex)
            {
                ...
                return new GetPullRequestResponse { ResponseError = $"Failed to get pull request. Error: {ex.Message}" };
            }
        }

        private async Task<PullRequestDetails> BuildPullRequestDetailsAsync(string repoOwner, string repoName, int pullRequestNumber, string repoPath, CancellationToken ct)
        {
