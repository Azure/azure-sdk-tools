...
Within this repository there are two certificates:

1. `mac_windows_cert.pfx`: generated by `dotnet dev-certs https`. Installable on `mac/windows`.
2. `ubuntu_cert.pfx`: generated on a `Ubuntu` distribution using `openssl`.

The key issue here is that `dotnet dev-certs` generated certificates are _not_ acceptable to a standard ubuntu distro. The issue is that the `KeyUsage` field in the `.crt` [MUST contain](https://github.com/dotnet/aspnetcore/issues/7246#issuecomment-541165030) the `keyCertSign` flag. Certificates generated by `dotnet dev-certs` do NOT have this flag. This means that if you're on Windows AND running the Ubuntu docker image, you will need to trust the `ubuntu_cert.pfx` locally prior to `docker run`.
...
This will be automatically retrieved if you run the nuget installed version of the tool.

#### Option 2

Import the appropriate already existing certificate within the `tools/test-proxy/docker/` folder.

### Docker Image + SSL

To connect to the docker on SSL, both the docker image and your local machine must trust the same SSL cert. The docker image already has `ubuntu_cert.pfx` imported and trusted, so all that is necessary is to trust that same cert prior to `docker run`.

In the future, passing in a custom cert via a bound volume that contains your certificate will be a possibility as well.

For additional reading on this process for trusting SSL certs locally, feel free to read up [here.](https://devblogs.microsoft.com/aspnet/configuring-https-in-asp-net-core-across-different-platforms/)
...
Use either local or docker image to start the tool. Reference the [Installation](#installation) section of this readme.
...
Very much like record, except now `x-recording-mode` header should be set to `playback`.
