...
- Use structured logging with appropriate levels: `LogInformation` for business events, `LogDebug` for diagnostics. Avoid string interpolation, use `logger.LogInformation("Message: {variable}")` format. Write debug logging to stderr and/or a file in MCP mode to avoid the misleading "FAILURE TO PARSE MESSAGE" type errors in the MCP client logs
