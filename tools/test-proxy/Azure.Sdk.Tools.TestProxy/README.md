...
> docker run -v <your-volume-name-or-location>:/etc/testproxy -p 5001:5001 -p 5000:5000 azsdkengsys.azurecr.io/engsys/ubuntu_testproxy_server:897574
```
...
POST to the Proxy Server. Pass in your "test-id". **This should be the path to your testfile + test name.**D
...
Check response header `x-recording-id` to get the recording-id for usage later on.
