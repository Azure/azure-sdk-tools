...
namespace APIViewWeb.Managers
{
    public class APIRevisionsManager : IAPIRevisionsManager
    {
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                if (review != null)
                {
                    ...
                    if (apiRevision != null)
                    {
                        ...
                        if (apiRevision.APIRevisionType == APIRevisionType.Automatic)
                        {
                            await CarryForwardMatchingRevisionDataAsync(apiRevision, codeFile, file.ContentHash);
                        }
        ...
        /// <summary>
        /// Finds prior revisions with the same API surface and carries their approval and
        /// auto-generated comment state forward to the target revision.
        /// </summary>
        /// <param name="targetRevision">The revision receiving matching revision data.</param>
        /// <param name="codeFile">The finalized code file used for API surface comparison.</param>
        /// <param name="contentHash">The finalized API surface hash.</param>
        private async Task CarryForwardMatchingRevisionDataAsync(APIRevisionListItemModel targetRevision, CodeFile codeFile, string contentHash)
        {
            var candidates = (await _apiRevisionsRepository.GetAPIRevisionsAsync(targetRevision.ReviewId))
                .Where(r => r.Id != targetRevision.Id && (r.IsApproved || r.HasAutoGeneratedComments));
            var renderedCodeFile = new RenderedCodeFile(codeFile);

            foreach (var sourceRevision in candidates)
            {
                if (targetRevision.IsApproved && targetRevision.HasAutoGeneratedComments)
                {
                    break;
                }

                if (await AreAPIRevisionsTheSame(sourceRevision, renderedCodeFile, incomingContentHash: contentHash))
                {
                    await CarryForwardRevisionDataAsync(targetRevision, sourceRevision);
                }
            }
        }
        ...
        public async Task UpdateAPIRevisionCopilotDetailsAsync(APIRevisionListItemModel revision, bool copilotReviewInProgress, string copilotReviewJobId = null, bool? hasAutoGeneratedComments = null)
        {
            await _apiRevisionsRepository.UpdateAPIRevisionCopilotDetailsAsync(revision.ReviewId, revision.Id, copilotReviewInProgress, copilotReviewJobId, hasAutoGeneratedComments);
            revision.CopilotReviewInProgress = copilotReviewInProgress;
            if (copilotReviewJobId != null)
            {
                revision.CopilotReviewJobId = copilotReviewJobId;
            }
            if (hasAutoGeneratedComments.HasValue)
            {
                revision.HasAutoGeneratedComments = hasAutoGeneratedComments.Value;
            }
        }
