...
flowchart TB
    subgraph system["Azure SDK AI Bot System"]
        ...
        code["Code Repository Sync<br/>(Python)<br/>azure-sdk-qa-bot-code-search/"]
    end
...
### 6. Code Repository Sync (`azure-sdk-qa-bot-code-search/`)

A daily synchronization job that shallow-checks out tenant-configured Git repositories and recursive submodules, then reconciles their current source files into stable Azure Blob paths. The Chat Agent reads this mirror through Agent Framework's built-in read-only file tools.
