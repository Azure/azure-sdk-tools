...
namespace APIViewUnitTests
{
    public class CopilotPollingBackgroundHostedServiceTests
    {
            ...
            string responseStatus, bool shouldProcessComments)
        {
            ...
            _mockApiRevisionsManager.Verify(x => x.UpdateAPIRevisionCopilotDetailsAsync(
                It.Is<APIRevisionListItemModel>(api => api.Id == "revision-123" && api.ReviewId == "review-456"),
                false,
                It.IsAny<string>(),
        ...
        [Fact]
        public async Task ProcessJobAsync_SuccessWithNoValidComments_StillMarksRevisionReviewed()
        {
            // Arrange
            AIReviewJobInfoModel jobInfo = CreateTestJobInfo();
            var pollResponse = new AIReviewJobPolledResponseModel
            {
                ...
                Comments = new List<AIReviewComment>() // No comments returned
            };

            SetupHttpClientForSuccessfulPolling(pollResponse);

            APIRevisionListItemModel updatedRevision = null;
            bool? capturedCopilotInProgress = null;
            bool? capturedHasAutoGeneratedComments = null;
            _mockApiRevisionsManager
                .Setup(x => x.UpdateAPIRevisionCopilotDetailsAsync(It.IsAny<APIRevisionListItemModel>(), It.IsAny<bool>(), It.IsAny<string>(), It.IsAny<bool?>()))
                .Callback<APIRevisionListItemModel, bool, string, bool?>((rev, inProgress, _, hasComments) =>
                {
                    updatedRevision = rev;
                    capturedCopilotInProgress = inProgress;
                    capturedHasAutoGeneratedComments = hasComments;
                })
            ...
            Assert.False(capturedCopilotInProgress);
            Assert.True(capturedHasAutoGeneratedComments);
        }
