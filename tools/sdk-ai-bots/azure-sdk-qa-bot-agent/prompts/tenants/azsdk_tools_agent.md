...
- **Permission failures**: for Azure DevOps access errors, direct the user to request or renew access through [Azure SDK access](https://aka.ms/azsdk/access). Do not tell them to request the permission named in the error or change ACLs unless authoritative evidence confirms a resource-specific ACL issue.
