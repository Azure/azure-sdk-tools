...
export async function generateRLCInPipeline(options: {
    ...
    if (options.typespecProject) {
        ...
        if (!options.skipGeneration) {
            logger.info(
                `Start to generate rest level client SDK from '${options.typespecProject}'.`,
            );
            ...
            if (options.sdkGenerationType === "command") {
                ...
                const installCommand =
                    prepareCommandToInstallDependenciesForTypeSpecProject(
        ...
        if (!options.skipGeneration) {
            ...
            if (!!options.autorestConfig) {
                ...
                logger.info(
                    `Autorest configuration is not found in spec PR comment, and start to find it in sdk repository.`,
                );
                ...
                for (const rp of fs.readdirSync(sdkFolderPath)) {
                    ...
                    if (fs.lstatSync(rpFolderPath).isDirectory()) {
                        for (const packageFolder of fs.readdirSync(
                            ...
                            if (
                                !fs.lstatSync(packageFolderPath).isDirectory()
                            ) {
                                continue;
                            }
                            ...
                            const requireFoundOnlyOne =
                                regexExecResult && regexExecResult.length === 2;
    ...
    try {
        ...
        logger.info(
            `Start to generate some other files for '${packageName}' in '${packagePath}'.`,
        );
        if (!options.skipGeneration) {
            ...
            if (isRushRepo(options.sdkRepo)) {
                await changeRushJson(
                    options.sdkRepo,
                    packageName,
                    getRelativePackagePath(packagePath),
                    "client",
                );
            }
        ...
        if (
            ...
            if (
                options.runningEnvironment === RunningEnvironment.SdkGeneration
            ) {
        ...
        if (isRushRepo(options.sdkRepo)) {
            ...
            execSync(
                `node common/scripts/install-run-rush.js build -t ${packageName} --verbose`,
                { stdio: "inherit" },
            );
            logger.info(
                `Start to run command 'node common/scripts/install-run-rush.js pack --to ${packageName} --verbose'.`,
            );
            execSync(
                `node common/scripts/install-run-rush.js pack --to ${packageName} --verbose`,
                { stdio: "inherit" },
            );
        } else {
            ...
            // To build generated codes except test and sample, we need to change tsconfig.json.
            execSync(`pnpm build --filter ${packageName}`, {
                stdio: "inherit",
            });
            logger.info(
                `Start to run command 'pnpm pack ' under ${packagePath}.`,
            );
        ...
        if (
            ...
            for (const file of fs.readdirSync(packagePath)) {
                if (file.startsWith("azure-rest") && file.endsWith(".tgz")) {
                    outputPackageInfo.artifacts.push(
                        path.join(relativePackagePath, file),
                    );
                }
            }
