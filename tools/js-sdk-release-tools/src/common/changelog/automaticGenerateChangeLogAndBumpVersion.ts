...
import {
    ...
    makeChangesForPatchReleasingTrack2,
...
export async function generateChangelogAndBumpVersion(
    ...
    logger.info(
        `Start to generate changelog and bump version in ${packageFolderPath}`,
    );
    ...
    const isStableRelease = await isStableSDKReleaseType(
        apiVersionType,
        options,
    );
    ...
    if (
        ...
        logger.info(
            `Generated changelogs and setting version for first${!npmViewResult ? " " : " stable"} release successfully`,
        );
    } else {
        if (!stableVersion) {
        ...
        try {
            ...
            if (
                (sdkType && sdkType === "mgmt") ||
                clientType === SDKType.RestLevelClient
            ) {
                logger.info(
                    `Package ${packageName} released before is track2 sdk.`,
                );
                logger.info("Start to generate changelog by comparing api.md.");
                const npmPackageRoot = path.join(
                    packageFolderPath,
                    "changelog-temp",
                    "package",
                );
                ...
                if (!fs.existsSync(apiMdFileNPM)) {
                    ...
                    await tryCreateLastestStableNpmViewFromGithub(
                        lastestStableApiView,
                    );
                }
                ...
                console.log(
                    "🚀 ~ generateChangelogAndBumpVersion ~ changelog:",
                    changelog.content,
                );
                ...
                if (nextVersion) {
                    shell.cd(path.join(packageFolderPath, "changelog-temp"));
                    shell.mkdir(
                        path.join(packageFolderPath, "changelog-temp", "next"),
                    );
                    shell.cd(
                        path.join(packageFolderPath, "changelog-temp", "next"),
                    );
                    ...
                    const latestDate = getversionDate(
                        npmViewResult,
                        stableVersion,
                    );
                ...
                if (
                    originalChangeLogContent.includes(
                ...
                originalChangeLogContent = fixChangelogFormat(
                ...
                if (!changelog.hasBreakingChange && !changelog.hasFeature) {
                    ...
                    const oriVersionReleased = !usedVersions
                        ? false
                        : usedVersions.includes(oriVersion);
                    ...
                    logger.info(
                        "Generated changelogs and set version for track2 release successfully.",
                    );
                    return changelog;
                }
            } else {
                logger.info(
                    `Package ${packageName} released before is track1 sdk.`,
                );
                logger.info(
                    "Start to generate changelog of migrating track1 to track2 sdk.",
                );
                const newVersion = getNewVersion(
                    stableVersion,
                    usedVersions,
            ...
            if (await exists(tempFolderPath)) await remove(tempFolderPath);
