...
describe("filterPullRequestData", () => {
    it("processes the full fixture as expected", () => {
        ...
        assert.deepEqual(kept.map((k) => k.user).sort(), [
            "carol",
            "carol",
            "eve",
        ]);
    ...
    it("includeSelf=true keeps alice's substantive comments", () => {
        ...
        assert.ok(
            kept.some((k) => k.user === "alice" && k.body.includes("rebase")),
        );
    });
});
...
describe("classify author association", () => {
    ...
    it("covers only people formally joined to the repo", () => {
        assert.deepEqual([...ALLOWED_ASSOCIATIONS].sort(), [
            "COLLABORATOR",
            "MEMBER",
            "OWNER",
        ]);
...
describe("classify with automation markers", () => {
    it("drops bodies containing the Copilot CLI marker as bot", () => {
        const c = {
            body: "Auto-generated PR description with detail <!-- #comment-cli-pr -->",
            user: { login: "alice", type: "User" as const },
        };
        assert.equal(
            classifyComment({
...
describe("shouldSkipPr", () => {
    ...
    it("PR without mergedAt is skipped when --since is set", () => {
        const unmerged: PullRequestData = {
            ...base,
            pr: { ...base.pr, mergedAt: null },
        };
        assert.equal(
...
describe("filterPullRequestData with new filters", () => {
    ...
    it("drops bodies containing the Copilot CLI marker as bot through the full pipeline", () => {
        const pullRequestData: PullRequestData = {
            ...
            issue: [
                {
                    id: 4002,
                    body: "This PR was generated by an automation tool. <!-- #comment-cli-pr -->",
                    created_at: "2026-06-01T00:01:00Z",
    ...
    it("drops external feedback and counts association by default", () => {
        ...
        const { kept, dropped } = filterPullRequestData(
            pullRequestData,
            DEFAULTS,
        );
        assert.equal(kept.length, 1);
...
describe("parseArgs with new flags", () => {
    ...
    it("--source parses comma-separated sources", () => {
        const opts = parseArgs([sampleFile, "--source", "inline,issue"]);
    ...
    it("--max-body-length validates positive integer", () => {
        const opts = parseArgs([sampleFile, "--max-body-length", "100"]);
