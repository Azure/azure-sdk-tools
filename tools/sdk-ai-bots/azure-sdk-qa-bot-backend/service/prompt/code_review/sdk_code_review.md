...
Please note if we have an name with Bar and BarAutoGenerated, which usually means there are two models with the same name so we rename the latter as BarAutoGenerated. In that case it's better to rename one of them or unify the duplication.
