...
) : MCPTool
{
    ...
    {
        var command = new Command(AnalyzeCommandName, "Analyze a log file for errors and issues");
        command.Options.Add(filePathOpt);
        command.Options.Add(keywordsOpt);
        command.Options.Add(fullSearchOpt);
        command.Options.Add(contextLinesOpt);
        return command;
    }
