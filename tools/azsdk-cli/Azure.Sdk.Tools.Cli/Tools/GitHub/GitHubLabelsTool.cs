...
namespace Azure.Sdk.Tools.Cli.Tools.GitHub
{
    ...
    ) : MCPMultiCommandTool
    {
        ...
        {
            var checkCommand = new Command(checkServiceLabelCommandName, "Check if a service label exists in the common labels CSV");
            checkCommand.Options.Add(serviceLabelOpt);

            var createCommand = new Command(createServiceLabelCommandName, "Creates a PR for a new label given a proposed label and brand documentation.");
            createCommand.Options.Add(serviceLabelOpt);
            createCommand.Options.Add(documentationLinkOpt);

            return [checkCommand, createCommand];
        }
