    ...
    if (options.typespecProject) {
        ...
        if (!options.skipGeneration) {
            ...
            if(options.sdkGenerationType === "command") {
                ...
                const scriptCommand = ['tsp-client', 'init', '--update-if-exists', '--debug', '--tsp-config', path.join(tspDefDir, 'tspconfig.yaml'), '--local-spec-repo', tspDefDir, '--repo', generateRepoDataInTspLocation(options.swaggerRepoUrl), '--commit', options.gitCommitId].join(" ");
