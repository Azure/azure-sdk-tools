    ...
    public class McpLabelerEvaluator
    {
            ...
            bool stopOnFirstError = false)
        {
            ...
            foreach (var testCase in testCases)
            {
                ...
                {
                    _logger.LogInformation($"  Server: {result.PredictedServerLabel ?? "NONE"} (Expected: {testCase.ExpectedServerLabel}) - {(result.ServerCorrect ? "✓" : "✗")}");
                    _logger.LogInformation($"  Tool: {result.PredictedToolLabel ?? "NONE"} (Expected: {testCase.ExpectedToolLabel}) - {(result.ToolCorrect ? "✓" : "✗")}");
                    _logger.LogInformation($"  RAG Results: {result.RagResultCount}");
                }
        ...
        private async Task<McpPredictionResult> EvaluateSingleAsync(McpTestCase testCase)
        {
            ...
            try
            {
                ...
                // Note: RAG result count and confidence would need to be exposed by McpOpenAiLabeler
                // For now, we can't capture these without modifying the labeler
                result.RagResultCount = 0; // Placeholder
            }
            catch (Exception ex)
            {
                result.Error = ex;
        ...
        private McpEvaluationMetrics CalculateMetrics(List<McpPredictionResult> results)
        {
            ...
            metrics.AverageRagResults = successfulResults.Any()
                ? successfulResults.Average(r => r.RagResultCount)
                : 0;

            metrics.ZeroRagResults = successfulResults.Count(r => r.RagResultCount == 0);
        ...
        public void ExportToCsv(List<McpPredictionResult> results, string filePath)
        {
            ...
            // Header
            writer.WriteLine("IssueNumber,Title,ExpectedServer,PredictedServer,ServerCorrect,ExpectedTool,PredictedTool,ToolCorrect,BothCorrect,RagResults,Error");
