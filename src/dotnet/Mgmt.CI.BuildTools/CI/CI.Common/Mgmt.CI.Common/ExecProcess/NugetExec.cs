    ...
    public class NugetExec : ShellExec
    {
        ...
        public string PublishToPath
        {
            get
            {
                ...
                {
                    _publishToPath = CommonConstants.NugetDefaults.NUGET_PUBLISH_URL;
                }
        ...
        public string PublishSymbolToPath
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(_publishSymbolToPath))
                {
                    ...
                    {
                        _publishSymbolToPath = CommonConstants.NugetDefaults.NUGET_SYMBOL_PUBLISH_URL;
                    }
                    else if (PublishToPath.Contains(CommonConstants.NugetDefaults.NUGET_PUBLISH_URL))
                    {
                        _publishSymbolToPath = CommonConstants.NugetDefaults.NUGET_SYMBOL_PUBLISH_URL;
                    }
                    else
        ...
        public string NugetExePath
        {
            get
            {
                ...
                {
                    _nugetExePath = CommonConstants.NugetDefaults.NUGET_PATH;
                }
        ...
        public NugetExec() : this(CommonConstants.NugetDefaults.NUGET_PATH)
        {
        }
        ...
        public NugetPublishStatus Publish(string nupkgPath)
        {
            ...
            {
                args = string.Format("push {0} -source {1} -NonInteractive -Timeout {2} -SymbolSource {3} -ApiKey {4} ", 
                    nupkgPath, PublishSymbolToPath, CommonConstants.NugetDefaults.NUGET_TIMEOUT, PublishSymbolToPath, ApiKey);
                displayArgs = string.Format("push {0} -source {1} -NonInteractive -Timeout {2} -SymbolSource {3} -ApiKey {4} ", 
                    nupkgPath, PublishSymbolToPath, CommonConstants.NugetDefaults.NUGET_TIMEOUT, PublishSymbolToPath, "<ApiKey>");
            }
            else
            {
                args = string.Format(_defaultNugetArgsFormat, nupkgPath, PublishToPath, ApiKey, CommonConstants.NugetDefaults.NUGET_TIMEOUT);
                displayArgs = string.Format("{0} push {1} -source {2} -ApiKey {3} -NonInteractive -Timeout {4}", 
                    NugetExePath, nupkgPath, PublishToPath, "<ApiKey>", CommonConstants.NugetDefaults.NUGET_TIMEOUT);
            }
