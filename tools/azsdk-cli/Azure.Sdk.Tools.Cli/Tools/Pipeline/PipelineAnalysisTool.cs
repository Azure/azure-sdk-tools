...
) : MCPTool
{
    ...
    {
        var command = new Command("analyze", "Analyze a pipeline run");
        command.Arguments.Add(pipelineArg);
        command.Options.Add(projectOpt);
        command.Options.Add(logIdOpt);
        command.Options.Add(analyzeWithAgentOpt);
        command.Options.Add(projectEndpointOpt);
        command.Options.Add(aiModelOpt);
