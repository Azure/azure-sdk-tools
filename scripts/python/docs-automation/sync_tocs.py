...
def filter_children(targeted_ns_list, known_namespaces):
    ...
    for ns in targeted_ns_list:
        ...
        if any([re.match(fnmatch.translate(ns), known_namespace) for known_namespace in known_namespaces]):
...
def filter_toc(toc_dict, namespaces, repo_location = None, readme_suffix = None):
    ...
    if ("items" in toc_dict):
        ...
        if repo_location and readme_suffix:
            if "href" in toc_dict:
                toc_dict["href"] = amend_href(toc_dict["href"], repo_location, readme_suffix)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Execution requires that both the known namespaces and reference yml be defined.")

    present_in_target = grep_children_namespaces(target_autogenerated_toc)

    print("Here are the visible namespaces in target autogenerated ToC. Constraining reference.yml.")
