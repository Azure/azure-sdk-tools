...
| C4757 | do-not-import-asyncio | If the only usage of asyncio in code is for sleep, use the sleep function from the correct azure core transport. Otherwise, if asyncio usage can not be determined or fixed disable this warning. | pylint:disable=do-not-import-asyncio | No Link. |
