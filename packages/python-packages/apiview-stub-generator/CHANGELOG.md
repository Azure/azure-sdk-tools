...
Raised the package-install timeout in the API-stub generator from 120s to 300s and made it configurable via the `APISTUB_INSTALL_TIMEOUT` environment variable. Packages with large dependency trees (e.g. those pulling the Microsoft OpenTelemetry distro) could exceed the previous 120s limit on slower CI agents even though the install was healthy, causing spurious API Review failures.
