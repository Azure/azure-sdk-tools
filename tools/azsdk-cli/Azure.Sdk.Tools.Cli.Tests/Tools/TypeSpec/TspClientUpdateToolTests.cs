...
//         public string GetCustomizationRootAsynction(ClientUpdateSessionState session, string generationRoot, CancellationToken ct) => Task.FromResult<string?>(null); // none
//         public Task<(List<CustomizationImpact> impacts, List<PatchProposal> patches)> AnalyzeAndProposePatchesAsync(ClientUpdateSessionState session, string? customizationRoot, IEnumerable<ApiChange> apiChanges, CancellationToken ct) => Task.FromResult((new List<CustomizationImpact>(), new List<PatchProposal>()));
...
//         public Task<PatchApplicationResult> ApplyPatchesAsync(ClientUpdateSessionState session, IEnumerable<PatchProposal> patches, CancellationToken ct) => Task.FromResult(new PatchApplicationResult { Success = true });
//     }
...
//             });
//         public string GetCustomizationRootAsynction(ClientUpdateSessionState session, string generationRoot, CancellationToken ct) => Task.FromResult<string?>(null); // none
//         public Task<(List<CustomizationImpact> impacts, List<PatchProposal> patches)> AnalyzeAndProposePatchesAsync(ClientUpdateSessionState session, string? customizationRoot, IEnumerable<ApiChange> apiChanges, CancellationToken ct)
//             => Task.FromResult((new List<CustomizationImpact> { new CustomizationImpact { File = "Customization.java", Description = "API change S1", ImpactType = "MethodAdded", Severity = "Low" } }, new List<PatchProposal>()));
...
//             => Task.FromResult(new List<CustomizationImpact> { new CustomizationImpact { File = "Customization.java", Description = "API change S1", ImpactType = "MethodAdded", Severity = "Low" } });
//         public Task<List<PatchProposal>> ProposePatchesAsync(ClientUpdateSessionState session, IEnumerable<CustomizationImpact> impacts, CancellationToken ct) => Task.FromResult(new List<PatchProposal>());
//         public Task<ValidationResult> ValidateAsync(ClientUpdateSessionState session, CancellationToken ct) => Task.FromResult(ValidationResult.CreateSuccess());
//         public Task<List<PatchProposal>> ProposeFixesAsync(ClientUpdateSessionState session, IEnumerable<string> validationErrors, CancellationToken ct) => Task.FromResult(new List<PatchProposal>());
//         public Task<PatchApplicationResult> ApplyPatchesAsync(ClientUpdateSessionState session, IEnumerable<PatchProposal> patches, CancellationToken ct) => Task.FromResult(new PatchApplicationResult { Success = true });
...
//         public string SupportedLanguage => "java";
//         public Task<List<ApiChange>> DiffAsync(string oldGeneratedPath, string newGeneratedPath) => Task.FromResult(new List<ApiChange>());
//         public string GetCustomizationRootAsynction(ClientUpdateSessionState session, string generationRoot, CancellationToken ct) => Task.FromResult<string?>(null);
//         public Task<(List<CustomizationImpact> impacts, List<PatchProposal> patches)> AnalyzeAndProposePatchesAsync(ClientUpdateSessionState session, string? customizationRoot, IEnumerable<ApiChange> apiChanges, CancellationToken ct) => Task.FromResult((new List<CustomizationImpact>(), new List<PatchProposal>()));
//         public Task<List<CustomizationImpact>> AnalyzeCustomizationImpactAsync(ClientUpdateSessionState session, string? customizationRoot, IEnumerable<ApiChange> apiChanges, CancellationToken ct) => Task.FromResult(new List<CustomizationImpact>());
//         public Task<List<PatchProposal>> ProposePatchesAsync(ClientUpdateSessionState session, IEnumerable<CustomizationImpact> impacts, CancellationToken ct) => Task.FromResult(new List<PatchProposal>());
