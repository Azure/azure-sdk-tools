    ...
    async def search_knowledge_base(
        ...
        *,
        queries: Annotated[
            ...
            "Use a **mixed query strategy** — the knowledge base contains both "
            "documentation (indexed by topic/solution) AND historical Q&A "
            "(indexed by the original problem description). "
            "IMPORTANT: The first query MUST be a **short, problem-phrased "
            "query** (under 10 words) extracted from the user's question "
            "title or core ask. Use the user's own key phrase — do NOT "
            "add solution terms, decorator names, version numbers, or "
            "qualifiers. If the user's message has a title, prefer that. "
            "Good query strategy: "
            "1) **Short problem query** — the user's core question in under "
            "10 words. Strip everything except the essential nouns and verb. "
            "Example: user title 'How to version a spread property' → "
            ...
            "CODEOWNERS branch protection write access permissions onboarding'] "
            "  GOOD: ['cannot approve PR in azure-rest-api-specs', "
            "          'request access to Azure API and SDK repositories write access', "
