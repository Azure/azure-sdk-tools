...
namespace APIViewUnitTests
{
    public class AutoReviewServiceTests
    {
        ...
        [Fact]
        public async Task CreateAutomaticRevisionAsync_WithCompareAllRevisionsTrue_CreatesNewVersionAndCarriesApprovalForward()
        {
            ...
            var newRevision = new APIRevisionListItemModel
            {
                ...
                IsApproved = false,
                IsReleased = false,
                Files = new List<APICodeFileModel> { new APICodeFileModel { PackageVersion = "1.0.1" } }
            };
            ...
            _mockApiRevisionsManager.Setup(m => m.CarryForwardRevisionDataAsync(newRevision, approvedRevision))
                .Returns(Task.CompletedTask);
            ...
            _mockApiRevisionsManager.Verify(m => m.CarryForwardRevisionDataAsync(newRevision, approvedRevision), Times.Once);
            _mockApiRevisionsManager.Verify(m => m.UpdateAPIRevisionAsync(approvedRevision), Times.Never);
        ...
        public async Task CreateAutomaticRevisionAsync_ExactMatchUpdatesNonReleasedRevisionInPlace(
            bool isApproved,
            bool hasAutoGeneratedComments)
        {
            var codeFile = CreateCodeFile("TestPackage", "1.0.0", "C#");
            using var memoryStream = new MemoryStream();
            ...
            var existingRevision = new APIRevisionListItemModel
            {
                ...
                {
                    new APICodeFileModel { FileId = "old-file-id", PackageVersion = "1.0.0" }
                }
            };

            _mockReviewManager.Setup(m => m.GetReviewAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<bool?>()))
                .ReturnsAsync(new ReviewListItemModel { Id = "review-id", PackageName = "TestPackage", Language = "C#" });
            ...
            _mockApiRevisionsManager.Setup(m => m.AreAPIRevisionsTheSame(
                    existingRevision, It.IsAny<RenderedCodeFile>(), true, "test-hash"))
                .ReturnsAsync(true);
            _mockApiRevisionsManager.Setup(m => m.UpdateAPIRevisionAsync(existingRevision))
                .Returns(Task.CompletedTask);

            var (_, apiRevision) = await _service.CreateAutomaticRevisionAsync(
                _testUser, codeFile, "new-label", "new.json", memoryStream, null);

            apiRevision.Id.Should().Be("existing-revision-id");
            apiRevision.IsApproved.Should().Be(isApproved);
            apiRevision.HasAutoGeneratedComments.Should().Be(hasAutoGeneratedComments);
            _mockApiRevisionsManager.Verify(m => m.UpdateAPIRevisionAsync(existingRevision), Times.Once);
            _mockApiRevisionsManager.Verify(m => m.CreateAPIRevisionAsync(
                It.IsAny<string>(), It.IsAny<string>(), It.IsAny<APIRevisionType>(),
                It.IsAny<string>(), It.IsAny<MemoryStream>(), It.IsAny<CodeFile>(),
        ...
        [Fact]
        public async Task CreateAutomaticRevisionAsync_ExactReleasedMatchReturnsWithoutUpdating()
        {
            var codeFile = CreateCodeFile("TestPackage", "1.0.0", "C#");
            using var memoryStream = new MemoryStream();

            var releasedRevision = new APIRevisionListItemModel
            {
                Id = "released-revision-id",
                ReviewId = "review-id",
                APIRevisionType = APIRevisionType.Automatic,
                IsApproved = true,
                IsReleased = true,
            ...
            _mockReviewManager.Setup(m => m.GetReviewAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<bool?>()))
                .ReturnsAsync(new ReviewListItemModel { Id = "review-id", PackageName = "TestPackage", Language = "C#" });
            _mockApiRevisionsManager.Setup(m => m.GetAPIRevisionsAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<APIRevisionType>()))
                .ReturnsAsync(new List<APIRevisionListItemModel> { releasedRevision });
            _mockApiRevisionsManager.Setup(m => m.AreAPIRevisionsTheSame(
                    releasedRevision, It.IsAny<RenderedCodeFile>(), true, "test-hash"))
                .ReturnsAsync(true);

            var (_, apiRevision) = await _service.CreateAutomaticRevisionAsync(
                _testUser, codeFile, "new-label", "new.json", memoryStream, null);

            apiRevision.Should().BeSameAs(releasedRevision);
            _mockCodeFileManager.Verify(m => m.CreateReviewCodeFileModel(
                It.IsAny<string>(), It.IsAny<MemoryStream>(), It.IsAny<CodeFile>()), Times.Never);
            _mockApiRevisionsManager.Verify(m => m.UpdateAPIRevisionAsync(It.IsAny<APIRevisionListItemModel>()), Times.Never);
            _mockApiRevisionsManager.Verify(m => m.CreateAPIRevisionAsync(
                It.IsAny<string>(), It.IsAny<string>(), It.IsAny<APIRevisionType>(),
                It.IsAny<string>(), It.IsAny<MemoryStream>(), It.IsAny<CodeFile>(),
                It.IsAny<string>(), It.IsAny<int?>(), It.IsAny<string>()), Times.Never);
