...
export const gitSetRemoteWithAuth = async (
  context: Pick<SdkAutoContext_New, 'logger' | 'getGithubAccessToken'>,
  repo: SimpleGit,
  remoteName: string,
  remoteRepo: {
    owner: string;
...
export const gitRemoveAllBranches = async (context: SdkAutoContext_New, repo: SimpleGit) => {
...
export const gitGetDiffFileList = async (diff: string, context?: SdkAutoContext_New, description?: string) => {
...
export const gitCheckoutBranch = async (context: SdkAutoContext_New, repo: SimpleGit, branchName: string, reset: boolean = true) => {
  let headCommit: string | undefined = undefined;
  try {
    headCommit = await repo.revparse('HEAD');
  } catch (error) {
