...
- **Evaluations run with memory disabled.** The agent can inject historical Q&A from its memory store; because the eval datasets are built from prior Q&A, leaving memory on leaks ground-truth answers and biases the comparison. All evaluation is run with memory off. Because the `main` KB-only baseline predates the memory switch, the same gate is ported to `main` for the baseline run so **both arms are memory-off**; skipping this inflates KB-only substantially (measured at **+7.7 pp** under gpt-5.4, apispec **+31 pp** via leaked Q&A) and biases the comparison against the graph.
- **Result (link-neutralized 219-case perf set, memory off, gpt-5.4 grader).** The final configuration — hybrid KB+graph retrieval plus an answer **scope-discipline** rule (lead with the single direct verdict; keep graph context from over-expanding graph-backbone troubleshooting/process answers, not just definitional ones) — scores **66.2 %** vs **60.3 %** for the memory-off KB-only baseline, **+5.9 pp** (3-run mean; graph retrieval contributes ~+3.2 pp and the scope-discipline ~+2.7 pp). The retrieval gain comes from two reinforcing levers on the **graph path**: **(1) community-report synthesis** — surfacing GraphRAG's pre-computed cross-document community summary, the artifact the chunk-based KB cannot produce, which lifts the relational, onboarding, and release-support categories; and **(2) symmetric graph-reference ranking** (see [Hybrid synthesis](#4-hybrid-synthesis--how-the-two-paths-combine)), which removes the earlier graph dilution on definitional questions so the ranked graph set fuses cleanly with the tightly-ranked KB set. The scope-discipline is orthogonal to *which* conclusion the agent reaches, so it trims over-expansion losses without flipping the cases where graph correctly overrides KB, and answer length is essentially unchanged (the gain is from front-loading the correct verdict, not from shortening). Groundedness/relevance/coherence/fluency stay at ~100 %; `response_completeness` is the sole gating metric and tracks the headline rate.
- **Remaining failures are dominated by corpus gaps** — short, thread-specific facts that simply aren't in the indexed corpus, so no retrieval change can recover them. The next lever is **curated corpus expansion**, not further retrieval tuning. (A general "trade conciseness for coverage" answer prompt was evaluated separately and did **not** net-improve — a broader answer suppresses `response_completeness` about as much as it saves length; the scope-discipline lever above helps precisely because it is the opposite, tightening answers to the asked question rather than widening them.)
