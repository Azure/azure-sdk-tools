...
{
    /// <summary>
    /// This MCP-only tool fixes dependency conflicts found in SDK packages.
    /// </summary>
    [Description("Fix dependency conflicts in SDK packages")]
    [McpServerToolType]
    public class DependencyCheckFixTool : MCPTool
    {
        ...
        {
            Command command = new("dependencyCheckFix", "Return dependency check fix prompts for SDK packages");
        ...
        public override async Task HandleCommand(InvocationContext ctx, CancellationToken ct)
        {
            try
            {
                ...
                var result = await FixDependencyCheckValidation(packagePath);
        ...
        [McpServerTool(Name = "FixDependencyCheckValidation"), Description("Fix dependency conflicts in SDK packages. Provide absolute path to package root as param.")]
        public async Task<ICLICheckResponse> FixDependencyCheckValidation(string packagePath)
        {
            try
            {
                logger.LogInformation($"Starting dependency check fixes for package at: {packagePath}");
