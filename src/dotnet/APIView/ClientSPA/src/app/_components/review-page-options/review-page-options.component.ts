...
export class ReviewPageOptionsComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    private router: Router,  private apiRevisionsService: APIRevisionsService,
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['activeAPIRevision'] && changes['activeAPIRevision'].currentValue != undefined) {
      ...
      (this.activeAPIRevision?.hasAutoGeneratedComments) ? this.aiReviewGenerationState = 'Completed' : this.aiReviewGenerationState = 'NotStarted';
    }
  ...
  generateAIReview(event: Event) {
    ...
    this.apiRevisionsService.generateAIReview(this.activeAPIRevision!.reviewId, this.activeAPIRevision!.id, diffApiRevisionId).pipe(take(1)).subscribe({
      ...
      error: (error: any) => {
        ...
        this.messageService.add({ severity: 'error', icon: 'bi bi-info-circle', summary: 'AI Comments', detail: 'Failed to generate AI review', key: 'bl', life: 10000, closable: true });
      },
      complete: () => {
        if (noOfAICommentsCreated > 0) {
          ...
          const messagePart = (noOfAICommentsCreated === 1) ? "AI comment generated." : "AI comments generated.";
          const messageDetail = `${noOfAICommentsCreated} ${messagePart}. Refresh the page to see the comments.`;
          this.messageService.add({ severity: 'success', icon: 'bi bi-stars', summary: 'AI Comments', detail: messageDetail, data: messgaeData, key: 'bl', life: 10000, closable: true });
        }
        setTimeout(() => {
          this.aiReviewGenerationState = 'Completed';
          this.generateAIReviewButtonText = 'Regenerate AI review';
        }, 3000);
      }
    });
  }
