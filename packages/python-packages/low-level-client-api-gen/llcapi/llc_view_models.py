        ...
        for param_num in range(0, len(self.parameters)):
            ...
            # Create a new line for the next operation
                ...
                if self.json_response:
                    ...
                    self.add_new_line(1)
                    inp = request_builder(
                        self, self.json_response, self.yaml, notfirst=False
                    )
                    for i in inp:
                        if i:
        ...
        for i in range(0, len(json_request)):
            if isinstance(json_request[i], str):
                ...
                if inner_model:
                    ...
                    request_builder(
                        self, json_request[i], yaml, indent=indent + 1, notfirst=True
                    )
