...
{
    private class DummyRepoFactory : ILanguageRepoServiceFactory
    {
    public ILanguageRepoService GetService(string packagePath, IProcessHelper processHelper, IGitHelper gitHelper, Microsoft.Extensions.Logging.ILogger logger)
            => throw new NotImplementedException();
        public ILanguageRepoService GetService(string packagePath)
            => throw new NotImplementedException();
        public string DetectLanguage(string repositoryPath) => "java";
    }
    ...
    private class TestLanguageServiceFailThenFix : IUpdateLanguageService
    {
        ...
        public Task<(bool success, List<string> errors)> ValidateAsync(UpdateSessionState session, CancellationToken ct)
        {
            ...
            if (attempt == 0)
            {
                return Task.FromResult((false, new List<string> { "compile error" }));
            }
