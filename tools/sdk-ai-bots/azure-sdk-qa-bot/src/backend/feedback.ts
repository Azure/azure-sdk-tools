...
export async function sendFeedback(conversation: string[], reaction: FeedbackReaction) {
  ...
  if (response.status !== 200) {
    throw new Error(`Failed to fetch data from feedback backend. Status: ${response.status}`);
  }
