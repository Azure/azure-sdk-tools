...
public class DotNetBreakingChangeClassificationTests
{
    ...
    private TestLogger<SdkBreakingChangeClassificationService> _logger = null!;

    [SetUp]
    public void SetUp()
    {
        _agentRunner = new Mock<ICopilotAgentRunner>();
        _logger = new TestLogger<SdkBreakingChangeClassificationService>();
        _service = new SdkBreakingChangeClassificationService(
            _agentRunner.Object, _logger);
    }
    ...
    public void Prompt_ExplainsValidFieldValuesOutsideTheJsonExample(string language)
    {
        ...
        Assert.That(prompt, Does.Contain("resolution is an optional one-line actionable mitigation instruction"));
        Assert.That(prompt, Does.Contain("Describe the customization to apply; classification does not execute it"));
        Assert.That(prompt, Does.Not.Contain("it is not a tool-execution instruction generated by this classifier"));
    }

    [TestCase("")]
    [TestCase(" \n")]
    public async Task Classify_WithoutPatternsWarnsAndStillReturnsActionableResolution(string patterns)
    {
        const string resolution = "Apply @@clientName(Widget, \"LegacyWidget\", \"go\") in the client customization.";
        ConfigureResponse(JsonSerializer.Serialize(new
        {
            hasBreakingChange = true,
            breakingChanges = new[]
            {
                new { breakingChange = "Widget renamed", category = "emitter change", resolution, originBreaks = new[] { "LegacyWidget removed" } },
            },
        }));

        var result = await _service.ClassifySdkBreakingChangesAsync(
            "### Breaking Changes\n- LegacyWidget removed", patterns, "Go", null, CancellationToken.None);

        Assert.That(result!.BreakingChanges.Single().Resolution, Is.EqualTo(resolution));
        Assert.That(_logger.Logs.Any(log => log.ToString()!.Contains("may reduce classification accuracy")), Is.True);
        _agentRunner.Verify(r => r.RunAsync(It.Is<CopilotAgent<string>>(agent => !agent.Tools.Any()),
