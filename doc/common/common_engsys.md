...
7. Review and approve each of your **Sync PRs**. For some repos (C and C++) you might need to frequently use the `Update Branch` button to get the checks green. You could run script `../../eng/scripts/Approve-Sync-PRs.ps1 <Tools PR number>` to know the status of all **Sync PRs** and automatically approve them. Note that the script doesn't merge the PRs.
