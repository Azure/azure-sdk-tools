...
public class CustomizedCodeUpdateTool : LanguageMcpTool
{
    ...
    private async Task<CustomizedCodeUpdateResponse> RunUpdateAsync(string packagePath, string? tspProjectPath, string customizationRequest, EditScope editScope, CancellationToken ct)
    {
        ...
        try
        {
            packageInfo = languageService != null ? await languageService.GetPackageInfo(packagePath, ct) : null;
        }
        ...
        try
        {
            ...
            //    {
            //        Success = false,
            //        ResponseError = "No feedback items found to classify.",
            //        Message = "No feedback items found to classify.",
            //        ErrorCode = CustomizedCodeUpdateResponse.KnownErrorCodes.InvalidInput,
            //        BuildResult = "No feedback items found to classify."
            //    });
            //}
            ...
            foreach (var itemDetails in classifications)
            {
                ...
                if (itemDetails.Classification == ClassificationTspApplicable)
                {
                    ...
                    var languageTaggedRequest = $"For {languageService?.Language.ToString() ?? string.Empty}: {itemDetails.Text}";
