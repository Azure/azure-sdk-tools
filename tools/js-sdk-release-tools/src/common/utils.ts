...
export async function applyLegacySettingsMapping(
    ...
    if (hasChanges) {
        ...
        try {
            ...
            // Update only the options section
            if (originalConfig && typeof originalConfig === 'object') {
                (originalConfig as any).options = updatedOptions.options;
...
export async function getGeneratedPackageDirectory(
    ...
    //logger.info(`Resolved options from tspconfig.yaml: ${JSON.stringify(tspConfig, undefined, 2)}`);
