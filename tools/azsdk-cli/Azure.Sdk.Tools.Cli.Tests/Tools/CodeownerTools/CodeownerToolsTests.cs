...
namespace Azure.Sdk.Tools.Cli.Tests.CodeownerToolsSuite
{
    ...
    internal sealed class FakeRepoContentList : IReadOnlyList<Octokit.RepositoryContent>
    {
        ...
        {
            for (int i = 0; i < _count; i++) yield return null!;
        }
    ...
    internal class CodeownerToolsTests
    {
            ...
            out ILabelHelper labelHelper)
        {
            ...
            labelHelper = new LabelHelper(new Mock<ILogger<LabelHelper>>().Object);

            // Default behaviors that are commonly used
            typespec.Setup(t => t.IsTypeSpecProjectForMgmtPlane(It.IsAny<string>())).Returns(false);

            return new CodeownerToolsType(gh.Object, output.Object, typespec.Object, helper, validator.Object, labelHelper);
        }
        ...
        {
            var sut = CreateSut(out var _, out var _, out var _, out var _, out var _, out var _);
        ...
        {
            var sut = CreateSut(out var gh, out var _, out var typespec, out var _, out var _, out var _);
            // Prevent any unintended calls beyond input validation
            gh.Setup(g => g.GetContentsAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
        ...
        {
            var sut = CreateSut(out var gh, out var _, out var _, out var _, out var _, out var _);

            // Codeowners file missing
            gh.Setup(g => g.GetContentsAsync(Constants.AZURE_OWNER_PATH, "azure-sdk-for-net", Constants.AZURE_CODEOWNERS_PATH))
              .ReturnsAsync((IReadOnlyList<Octokit.RepositoryContent>)null!);
        ...
        public async Task UpdateCodeowners_Fails_WhenLabelsFileMissing()
        {
            ...
            // Labels file missing
            gh.Setup(g => g.GetContentsAsync(Constants.AZURE_OWNER_PATH, Constants.AZURE_SDK_TOOLS_PATH, Constants.AZURE_COMMON_LABELS_PATH))
              .ReturnsAsync((IReadOnlyList<Octokit.RepositoryContent>)null!);
        ...
        {
            var sut = CreateSut(out var gh, out var _, out var _, out var helper, out var _, out var _);
            ...
            {
                gh.Setup(g => g.GetBranchAsync(Constants.AZURE_OWNER_PATH, "azure-sdk-for-net", workingBranch))
                  .ReturnsAsync(existingBranch);
            }
            else
            {
                gh.Setup(g => g.GetBranchAsync(Constants.AZURE_OWNER_PATH, "azure-sdk-for-net", It.IsAny<string>()))
                  .ReturnsAsync(existingBranch);
            }
            ...
            gh.Setup(g => g.CreatePullRequestAsync("azure-sdk-for-net", Constants.AZURE_OWNER_PATH, "main", It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), true))
              .ReturnsAsync(new List<string> { "Pull request created successfully as draft PR." });
            ...
            string expectedErrorContains)
        {
            ...
            var sut = CreateSut(out var _, out var _, out var _, out var _, out var validator, out var _);
        ...
        public async Task CreateCodeownerPR_WorkingBranchProvidedButMissing_CreatesNewBranch()
        {
            ...
            gh.Setup(g => g.GetBranchAsync(Constants.AZURE_OWNER_PATH, "azure-sdk-for-net", "feature/missing"))
              .ReturnsAsync(false);
            gh.Setup(g => g.CreateBranchAsync(Constants.AZURE_OWNER_PATH, "azure-sdk-for-net", It.IsAny<string>(), "main"))
              .ReturnsAsync(CreateBranchStatus.Created);
            gh.Setup(g => g.UpdateFileAsync(Constants.AZURE_OWNER_PATH, "azure-sdk-for-net", Constants.AZURE_CODEOWNERS_PATH, It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
        ...
        public async Task ValidateCodeOwnerEntryForService_Validates_ForRepoPath_WithMockedOwners()
        {
            ...
            Assert.That(
                ...
                Is.True,
                $"Unexpected validation message: {result.Message}");
        }

        [Test, Category("Integration")]
        public async Task UpdateCodeowners_AddNewEntry_ByNewServiceLabel_Succeeds()
        {
            var sut = CreateSut(out var gh, out var _, out var typespec, out var _, out var validator, out var _);
            typespec.Setup(t => t.IsTypeSpecProjectForMgmtPlane(It.IsAny<string>())).Returns(false);

            // Real CODEOWNERS content so indices align when inserting
            var repoName = "azure-sdk-for-net";
            gh.Setup(g => g.GetContentsAsync(Constants.AZURE_OWNER_PATH, repoName, Constants.AZURE_CODEOWNERS_PATH))
            ...
            // Branch/PR flow
            gh.Setup(g => g.GetBranchAsync(Constants.AZURE_OWNER_PATH, repoName, It.IsAny<string>())).ReturnsAsync(false);
        ...
        [Test, Category("Integration")]
        public async Task UpdateCodeowners_RemoveOwners_FromExistingEntry_Succeeds()
        {
            var sut = CreateSut(out var gh, out var _, out var typespec, out var helper, out var validator, out var _);
            typespec.Setup(t => t.IsTypeSpecProjectForMgmtPlane(It.IsAny<string>())).Returns(false);
            ...
            // Labels include a service label whose entry has >=2 AzureSdkOwners to satisfy validation (Tables)
            var csv = "Tables,,e99695\n";
            gh.Setup(g => g.GetContentsAsync(Constants.AZURE_OWNER_PATH, Constants.AZURE_SDK_TOOLS_PATH, Constants.AZURE_COMMON_LABELS_PATH))
              .ReturnsAsync(BuildLabelsContent(csv));

            // All owners considered valid
            validator.Setup(v => v.ValidateCodeOwnerAsync(It.IsAny<string>(), It.IsAny<bool>()))
                     .ReturnsAsync((string username, bool _) => new CodeOwnerValidationResult
                     {
                         Username = username,
