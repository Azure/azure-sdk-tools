...
namespace Azure.Sdk.Tools.Cli.Tools.EngSys
{
    ...
    public class CodeownersTools : MCPMultiCommandTool
    {
        ...
        {
            var updateCommand = new Command(updateCodeownersCommandName, "Update codeowners in a repository");
            updateCommand.Options.Add(repoOption);
            updateCommand.Options.Add(isMgmtPlaneOption);
            updateCommand.Options.Add(pathOptionOptional);
            updateCommand.Options.Add(serviceLabelOption);
            updateCommand.Options.Add(serviceOwnersOption);
            updateCommand.Options.Add(sourceOwnersOption);
            updateCommand.Options.Add(isAddingOption);
            updateCommand.Options.Add(workingBranchOption);

            var validateCommand = new Command(validateCodeownersEntryCommandName, "Validate codeowners for an existing service entry");
            validateCommand.Options.Add(repoOption);
            validateCommand.Options.Add(serviceLabelOption);
            validateCommand.Options.Add(pathOptionOptional);

            return [updateCommand, validateCommand];
        }
