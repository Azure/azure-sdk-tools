...
## Deletion Process
- Before deleting any item (guideline, example, or memory), you MUST:
  1. Unlink all related items (remove all references to and from the item).
  2. Check if any examples or memories that were linked to this item are now orphaned (i.e., have no remaining links to guidelines or memories).
  3. If any such examples or memories are orphaned, delete them as well.
- Only after all unlinking and orphan cleanup is complete, delete the original item.
- Never delete review jobs via the agent. Review jobs are deleted programmatically ONLY.
"""
