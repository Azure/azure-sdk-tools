    ...
    async def search_knowledge_base(
        ...
        *,
        queries: Annotated[
            ...
            "QUERY 1 (REQUIRED) — the user's exact question, issue title, "
            "error message, or requested symbol as written. Preserve word order "
            "and every concrete token, including decorator names, model/property "
            "names, version numbers, rule IDs, check names, config keys, and "
            "repository names. For a follow-up, append the resolved subject "
            "without replacing the original wording. Do not normalize synonyms "
            "or remove sample values in this query. "
            "QUERY 2 (optional) — a normalized standalone restatement: resolve "
            "follow-up context and obvious synonyms, then drop conversation-"
            "specific sample values while keeping the underlying feature terms. "
