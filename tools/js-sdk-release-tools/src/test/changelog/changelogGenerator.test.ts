...
`;
    ...
    const items = getItemsByCategory(changelogItems, ChangelogItemCategory.ClassMethodRemoved);
    expect(items).toHaveLength(1);
    expect(items[0]).toBe('Removed operation DataProductClient.listGitHubAuth');
    // should NOT generate the generic "has a new signature" message
    expect(getItemsByCategory(changelogItems, ChangelogItemCategory.ClassChanged)).toHaveLength(0);
  });
...
`;
    ...
    const items = getItemsByCategory(changelogItems, ChangelogItemCategory.ClassMethodAdded);
    expect(items).toHaveLength(1);
    expect(items[0]).toBe('Added operation DataProductClient.listGitHubAuth');
  });
...
`;
    ...
    const items = getItemsByCategory(changelogItems, ChangelogItemCategory.ClassMethodSignatureChanged);
    expect(items).toHaveLength(1);
    expect(items[0]).toBe('Operation DataProductClient.get has a new signature');
  });
...
`;
    ...
    expect(getItemsByCategory(changelogItems, ChangelogItemCategory.ClassChanged)).toHaveLength(1);
    expect(getItemsByCategory(changelogItems, ChangelogItemCategory.ClassMethodRemoved)).toHaveLength(0);
  });

  test('Constructor parameter order change (same types) is detected as ClassChanged', async () => {
    const baselineApiView = `
\`\`\`ts
// @public
export class DataProductClient {
...
`;
    ...
    expect(getItemsByCategory(changelogItems, ChangelogItemCategory.ClassChanged)).toHaveLength(1);
    expect(getItemsByCategory(changelogItems, ChangelogItemCategory.ClassChanged)[0]).toBe(
