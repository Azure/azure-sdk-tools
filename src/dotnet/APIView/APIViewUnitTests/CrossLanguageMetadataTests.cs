...
public class CrossLanguageMetadataTests
{
    ...
    public void CrossLanguageMetadata_Should_Deserialize_From_Full_CodeFile_JSON()
    {
        ...
        using JsonDocument document = JsonDocument.Parse(fullCodeFileJson);
        JsonElement crossLanguageElement = document.RootElement.GetProperty("CrossLanguageMetadata");
        string crossLanguageJson = crossLanguageElement.GetRawText();

        CrossLanguageMetadata result = JsonSerializer.Deserialize<CrossLanguageMetadata>(crossLanguageJson,
