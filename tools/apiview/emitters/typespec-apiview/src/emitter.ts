...
async function loadMappingFile(program: Program, mappingPath?: string): Promise<any | undefined> {
  ...
  const resolvedPath = resolvePath(program.host.getCurrentDirectory(), mappingPath);
  try {
    const mappingFile = await program.host.readFile(mappingPath);
    return mappingFile;
  } catch (error) {
    reportDiagnostic(program, {
...
function createApiViewEmitter(program: Program, options: ResolvedApiViewEmitterOptions) {
  ...
  async function emitApiView() {
    ...
    for (const service of services) {
      ...
      const mapping = await loadMappingFile(options.mappingPath);
