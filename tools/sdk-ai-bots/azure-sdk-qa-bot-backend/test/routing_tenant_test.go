...
func TestRoutingTenant_General(t *testing.T) {
	...
	testcases := []struct {
		name            string
		content         string
		original_tenant model.TenantID
		expected_tenant model.TenantID
	}{
		{
			"SDK Validation - .NET",
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			routedTenantID, _ := service.RouteTenant(tc.original_tenant, nil, llmMessages)
			require.NotNil(t, routedTenantID)
			require.Equal(t, tc.expected_tenant, routedTenantID)
		})
	}
}
