...
This guide targets `@microsoft/vally-cli` 0.14.0. Azure SDK Tools requires Node.js `>=22.12.0` for Vally.
...
| **constraints.max_duration** | Hard wall-clock limit for a stimulus. Vally 0.14 enforces it; use a duration string with units and leave realistic headroom.                                                                                                  |
...
| A9  | An unused named environment has malformed `git` or incomplete `files` entries                                    | Fix or remove it: Vally 0.14 validates every named environment; each file needs `src` + `dest` and generated sources must exist before lint |
| A10 | Trusting lint to catch an executor typo                                                                          | Run a focused eval too: Vally 0.14 resolves executor names at runtime and reports the available executors there                             |
