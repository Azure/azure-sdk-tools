...
namespace APIViewLogicTest
{
    public class Program
    {
        ...
        private static void TestAreAllRelatedSDKReviewsApproved()
        {
            ...
            if (result2) throw new Exception("Expected false when some SDK reviews are not approved");

            // Test 3: No SDK reviews exist (only TypeSpec/Swagger)
            var reviews3 = new[]
            {
                CreateReview("contoso.widget", "TypeSpec", false),
            ...
            if (!result3) throw new Exception("Expected true when no SDK reviews exist");

            // Test 4: Mixed package names
            var reviews4 = new[]
            {
        ...
        private static bool AreAllRelatedSDKReviewsApproved(IEnumerable<(string PackageName, string Language, bool IsApproved)> allReviews, string packageBaseName)
        {
            var relatedSDKReviews = allReviews
                ...
                .Where(r => ExtractPackageBaseName(r.PackageName).Equals(packageBaseName, StringComparison.OrdinalIgnoreCase))
                .ToList();
