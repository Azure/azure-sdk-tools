...
**Azure DevOps Pipeline Analysis** — `azsdk_analyze_pipeline` for failure diagnosis. Use either `project` or `buildId` from ADO URLs. Read the `azsdk-common-pipeline-analysis` skill if necessary.
