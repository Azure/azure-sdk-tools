...
namespace APIViewWeb
{
    public class CosmosAPIRevisionsRepository : ICosmosAPIRevisionsRepository
    {
        ...
        public async Task UpdateAPIRevisionCopilotDetailsAsync(string reviewId, string apiRevisionId, bool copilotReviewInProgress, string copilotReviewJobId = null, bool? hasAutoGeneratedComments = null)
        {
            var lastUpdatedOn = DateTime.UtcNow;
            var patchOperations = new List<PatchOperation>
            {
                PatchOperation.Set("/CopilotReviewInProgress", copilotReviewInProgress),
                PatchOperation.Set("/LastUpdatedOn", lastUpdatedOn)
            };

            if (copilotReviewJobId != null)
            {
                patchOperations.Add(PatchOperation.Set("/CopilotReviewJobId", copilotReviewJobId));
            }

            if (hasAutoGeneratedComments.HasValue)
            {
                patchOperations.Add(PatchOperation.Set("/HasAutoGeneratedComments", hasAutoGeneratedComments.Value));
            }

            await _apiRevisionContainer.PatchItemAsync<APIRevisionListItemModel>(apiRevisionId, new PartitionKey(reviewId), patchOperations);
            await _reviewsRepository.UpdateReviewLastUpdatedOnAsync(reviewId, lastUpdatedOn);
        }
