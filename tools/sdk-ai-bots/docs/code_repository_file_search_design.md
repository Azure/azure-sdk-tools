...
- Prefer library/source declarations and tests when the same grep also matches samples; return samples as a fallback when no authoritative implementation match exists.
...
- Unit-test path normalization, read-only enforcement, manifest parsing, directory listing, direct reads, synchronized-root enforcement, declaration-first grep filtering, bounded grep, and per-operation manifest freshness.
