...
- **Handle the `graphrag_community` reference with care.** The graph tool may return one reference with `source="graphrag_community"` — a cross-document *overview* of a whole topic area, not a citable fact about one document. Use it **only** as connective framing on relational / process / cross-team questions where you need to see how things relate. For **specific, definitional, spec-validation, exact-rule, or single-concept questions** (e.g. API-spec PR mechanics, "is this Swagger/schema legal", a precise TypeSpec rule or check), it is background at most: **lead with the precise KB and graph text-unit evidence and never let the community overview generalise away, broaden, or dilute the exact answer.** If it pulls the answer toward a generic overview instead of the specific resolution, drop it.
