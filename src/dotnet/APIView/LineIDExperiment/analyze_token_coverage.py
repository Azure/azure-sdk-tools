...
def get_original_line_text(tokens: list) -> str:
    """Concatenate all token Values on a line without filtering token kinds."""
    parts = []
    for tok in tokens:
        val = tok.get("Value", "")
        if not val:
...
def walk_review_lines(lines: list, parent_text_chain: tuple = (),
    ...
    for line in lines:
        ...
        original_line_text = get_original_line_text(tokens)
        ...
        if line_id is not None and line_id != "":
            ...
            results.append({
                "line_id": line_id,
                "line_text": line_text,
                "original_line_text": original_line_text,
...
def analyze_file(filepath: str, verbose: bool = False, show_collisions: bool = False,
    ...
    key_examples = []
    for cid, grp in sorted(collision_groups.items(), key=lambda x: (-len(x[1]), x[0]))[:5]:
        sample = grp[0]
        key_examples.append({
            "calculated_id": cid,
            "group_size": len(grp),
            ...
            "original_line_text": sample.get("original_line_text", "")[:160],
...
def write_results_markdown(all_stats: list, source_paths: list, output_path: str):
    ...
    out_lines = []
    out_lines.append("# Token Coverage Results")
    out_lines.append("")
    out_lines.append(f"Input paths: {', '.join(source_paths)}")
    out_lines.append(f"Analyzed files: {total_files}")
    out_lines.append("")
    out_lines.append("## Methodology")
    out_lines.append("")
    out_lines.append(
        "Each line in a token file that carries a `LineId` is walked in tree order. "
        "A *calculated ID* is derived purely from the text content of the line and its ancestors: "
        "the text of each ancestor is sanitized (whitespace collapsed to `_`) and the chain is "
        "joined with `.` from root to the current line. "
        "If a child's sanitized text already begins with the parent's calculated ID, the parent prefix is not repeated."
    )
    out_lines.append("")
    out_lines.append(
        "A line is **reachable** if its text chain produces a non-empty calculated ID. "
        "A reachable line is **unique** when its calculated ID does not collide with any other line in the same file; "
        "otherwise it is **non-unique**. "
        "A line is **unreachable** if every token on it (after skipping keywords and punctuation) "
        "is empty or whitespace, so no discriminating text survives into the ID."
    )
    out_lines.append("")
    out_lines.append(
        "Examples of original lines that often become `empty-or-whitespace` after filtering: "
        "`@objcMembers`, `@get`, `@query`."
    )
    out_lines.append("")
    out_lines.append("## Executive Summary")
    out_lines.append("")
    if total_line_ids > 0:
        out_lines.append(f"- Total line IDs: {total_line_ids}")
        out_lines.append(f"- Unique reachable: {total_unique} ({total_unique/total_line_ids*100:.1f}%)")
        out_lines.append(f"- Non-unique reachable: {total_nonunique} ({total_nonunique/total_line_ids*100:.1f}%)")
        out_lines.append(f"- Unreachable: {total_unreachable} ({total_unreachable/total_line_ids*100:.1f}%)")
    else:
        out_lines.append("- No line IDs were analyzed.")
    out_lines.append("")

    out_lines.append("## Per-Language Averages")
    out_lines.append("")
    out_lines.append(
        "*Avg Non-Unique%* averages each file's rate equally. "
        "*Weighted Non-Unique%* is total non-unique lines ÷ total line IDs across all files in the language, "
        "so larger files carry more weight.*"
    )
    out_lines.append("")
    out_lines.append("| Language | Files | Avg Line IDs | Avg Unique% | Avg Non-Unique% | Avg Unreachable% | Weighted Non-Unique% |")
    out_lines.append("|---|---:|---:|---:|---:|---:|---:|")
    for lang in sorted(langs.keys()):
        items = langs[lang]
        n = len(items)
        avg_line_ids = sum(s["total_line_ids"] for s in items) / n
        ...
        out_lines.append(
            f"| {lang} | {n} | {avg_line_ids:.0f} | {avg_unique:.1f}% | {avg_nonunique:.1f}% | {avg_unreachable:.1f}% | {weighted_nonunique:.1f}% |"
        )
    out_lines.append("")

    out_lines.append("## Collision Classes By Language")
    out_lines.append("")
    out_lines.append("Collision class guide:")
    out_lines.append("")
    out_lines.append("- `comment-or-doc-boilerplate`: repeated doc/comment markers like `/**`, `*`, `*/`.")
    out_lines.append("- `path-like-identifier`: long dotted/qualified paths that repeat in many declarations.")
    out_lines.append("- `short-identifier`: short repeated names such as `from`, `value`, `status`.")
    out_lines.append("- `literal-numeric`: repeated numeric literals such as `0`, `0.0`, `1`.")
    out_lines.append("- `namespace-or-module-declaration`: repeated namespace/module/package declaration lines.")
    out_lines.append("- `annotation-or-attribute`: annotation-style lines beginning with `@`.")
    out_lines.append("- `empty-or-whitespace`: no surviving text after keyword/punctuation filtering.")
    out_lines.append("- `other`: collisions that do not match the buckets above.")
    out_lines.append("")
    for lang in sorted(langs.keys()):
        items = langs[lang]
        ...
        if nonunique_total == 0:
            out_lines.append("No non-unique collisions found.")
            out_lines.append("")
            continue

        out_lines.append("| Collision Class | Count | Share Of Language Non-Unique |")
        out_lines.append("|---|---:|---:|")
        for cls, count in class_counts.most_common(8):
            pct = count / nonunique_total * 100
            out_lines.append(f"| {cls} | {count} | {pct:.1f}% |")
        out_lines.append("")

        lang_examples = []
        for s in items:
            for ex in s.get("key_examples", []):
                lang_examples.append({"file": s["file"], **ex})
        lang_examples.sort(key=lambda e: (-e["group_size"], e["calculated_id"]))

        out_lines.append("Key examples:")
        out_lines.append("")
        out_lines.append("| Group Size | Class | Existing LineId | New Calculated LineId | Original Line | File |")
        out_lines.append("|---:|---|---|---|---|---|")
        for ex in lang_examples[:5]:
            existing_id = (ex.get("line_id", "") or "").replace("|", "\\|")
            cid = ex["calculated_id"].replace("|", "\\|")
            text = (ex.get("original_line_text", "") or "").replace("|", "\\|")
            out_lines.append(
                f"| {ex['group_size']} | {ex['collision_class']} | {existing_id} | {cid} | {text} | {ex['file']} |"
            )
        out_lines.append("")
