...
namespace Azure.Sdk.Tools.Cli.Tools.ReleasePlan
{
    ...
    ) : MCPMultiCommandTool
    {
        ...
        {
            var checkCommand = new Command(checkApiReadinessCommandName, "Check if API spec is ready to generate SDK");
            checkCommand.Options.Add(typeSpecProjectPathOpt);
            checkCommand.Options.Add(pullRequestNumberOpt);
            checkCommand.Options.Add(workItemIdOpt);

            var generateCommand = new Command(generateSdkCommandName, "Generate SDK for a TypeSpec project");
            generateCommand.Options.Add(typeSpecProjectPathOpt);
            generateCommand.Options.Add(apiVersionOpt);
            generateCommand.Options.Add(sdkReleaseTypeOpt);
            generateCommand.Options.Add(languageOpt);
            generateCommand.Options.Add(pullRequestNumberOpt);
            generateCommand.Options.Add(workItemIdOpt);

            var getSdkPrCommand = new Command(getSdkPullRequestCommandName, "Get SDK pull request link from SDK generation pipeline");
            getSdkPrCommand.Options.Add(languageOpt);
            getSdkPrCommand.Options.Add(pipelineRunIdOpt);
            getSdkPrCommand.Options.Add(workItemIdOpt);

            var linkCommand = new Command(linkSdkPrCommandName, "Link SDK pull request to release plan.");
            linkCommand.Options.Add(languageOpt);
            linkCommand.Options.Add(urlOpt);
            linkCommand.Options.Add(workItemOptionalIdOpt);
            linkCommand.Options.Add(releasePlanIdOpt);

            return [checkCommand, generateCommand, getSdkPrCommand, linkCommand];
        }
