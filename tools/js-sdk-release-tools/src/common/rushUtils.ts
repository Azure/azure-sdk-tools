...
async function addApiViewInfo(
  ...
  const [nodeApiViews, standardApiViews] = await Promise.all([glob(nodeApiViewPattern), glob(standardApiViewPattern)]);
...
export async function tryBuildSamples(packageDirectory: string, sdkRepoRoot: string, runMode: RunMode) {
