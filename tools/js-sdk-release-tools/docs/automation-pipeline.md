...
CLI Entry (autoGenerateInPipeline.ts)
  ...
  │   │   ├── pnpm install → customize → lint → build → changelog → pnpm pack
...
| Customize package   | `customizePackage()`                         | ⚠️ Advisory during migration         | `npm run customize` in the package directory              |
...
Each generated package defines its customization in the `customize` package script. The automation runs this script once after dependency installation and before lint or build. The command runs from the package directory.

Customization failures are advisory during the migration. The automation shows the npm output and logs a warning for failures such as a missing script, an unmet precondition, or a merge conflict. The automation then continues to lint and build. Build failure handling does not change.
...
| **7. Customize Package**                 | ⚠️ Advisory                           | `customizePackage()`                           | Run `npm run customize` once in the package directory. Log failures and continue during migration.                                                                                                                                                                                | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
| **8. Lint Fix**                          | ⚠️ Optional                           | `lintFix()` — only in `Release` / `Local` mode | `npm run lint:fix`                                                                                                                                                                                                                                                                 | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
| **9. Build**                             | ✅ Required                           | Compile package (excluding test/sample)        | `pnpm build --filter {packageName}...`                                                                                                                                                                                                                                             | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
| **10. Generate Changelog & Bump Version** | ✅ Required (non-skipGeneration)     | `generateChangelogAndBumpVersion()`            | Compare `api.md` between npm published version and local; detect breaking changes; generate changelog; bump version                                                                                                                                                                | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
| **11. Pack**                             | ✅ Required                           | Generate `.tgz` package                        | `pnpm run --filter {packageName}... pack`                                                                                                                                                                                                                                          | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
| **12. Update Snippets**                  | ✅ Required                           | `updateSnippets()`                             | `dev-tool run update-snippets`                                                                                                                                                                                                                                                     | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
| **13. Modify README**                    | ✅ Required (non-skipGeneration)      | `changeReadmeMd()`                             | Update package `README.md`                                                                                                                                                                                                                                                         | [generateMgmt.ts](../src/hlc/generateMgmt.ts) |
...
| **7. Customize Package**                  | ⚠️ Advisory | `customizePackage()`                        | Run `npm run customize` once in the package directory. Log failures and continue during migration.                    | [generateRLCInPipeline.ts](../src/llc/generateRLCInPipeline/generateRLCInPipeline.ts) |
...
| -------------------- | ----------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| pnpm install         | ✅ Required | `pnpm install`                                                                                    | [rushUtils.ts](../src/common/rushUtils.ts) |
| Customize package   | ⚠️ Advisory | Run `npm run customize` once for every package. Log failures and continue during migration.       | [rushUtils.ts](../src/common/rushUtils.ts) |
| Lint fix             | ⚠️ Optional | `npm run lint:fix` — only in `Release` / `Local` mode                                             | [rushUtils.ts](../src/common/rushUtils.ts) |
| turbo build          | ✅ Required | `pnpm turbo build --filter {packageName}... --token 1` (build errors are warnings for Data Plane) | [rushUtils.ts](../src/common/rushUtils.ts) |
