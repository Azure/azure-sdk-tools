...
> **Note on package-prefix stripping**: The Go SDK automatically removes the package name as a prefix from generated type names. For example, in the `webpubsub` package, a TypeSpec model named `WebpubsubResource` will be generated in Go as `Resource`. When searching for the matching Spec pattern or applying a `@@clientName` customization, you must use the **original TypeSpec name** (e.g., `WebpubsubResource`), not the prefix-stripped Go name (`Resource`).
...
```tsp
@@alternateType(Test.prop, "string", "go");
```
...
Locate the function parameter and use `@@alternateType` to change the parameter type back to the constant string:

```tsp
@@alternateType(Test.parameters.prop, "string", "go");
