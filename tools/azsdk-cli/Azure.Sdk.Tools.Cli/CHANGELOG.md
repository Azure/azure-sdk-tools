...
- Added `eng evaluate` CLI command and `azsdk_evaluate_pipeline_fixes` MCP tool to evaluate whether Copilot's fixes for failing pipelines took the pipeline from failure to success and survived into the merged pull request. Accepts a repository owner and name. Optional parameters: `--since-days`, `--model`.
