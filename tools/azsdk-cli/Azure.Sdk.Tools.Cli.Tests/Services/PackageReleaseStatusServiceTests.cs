...
public class PackageReleaseStatusServiceTests
{
    ...
    public async Task GetApprovalStatusAsync_UsesReviewHubResult_WhenReviewHubQuerySucceeds()
    {
        reviewHubServiceMock
            .Setup(x => x.GetReleaseGateStatusAsync("https://endpoint", "python", "pkg", "1.0.0", "client", "hash", "", It.IsAny<CancellationToken>()))
        ...
        var result = await service.GetApprovalStatusAsync("https://endpoint", "python", "pkg", "1.0.0", "client", "hash", "", CancellationToken.None);
    ...
    {
        reviewHubServiceMock
            .Setup(x => x.GetReleaseGateStatusAsync("https://endpoint", "java", "azure-keyvault-keys", "4.12.0b3", "client", "hash", "", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ApiReviewHubReleaseGateResult
            {
                IsApproved = false,
                Reason = "repositoryNotSupported"
        ...
        var result = await service.GetApprovalStatusAsync("https://endpoint", "java", "azure-keyvault-keys", "4.12.0b3", "client", "hash", "", CancellationToken.None);
    ...
    {
        reviewHubServiceMock
            .Setup(x => x.GetReleaseGateStatusAsync("https://endpoint", "cpp", "pkg", "1.0.0", "client", "hash", "", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ApiReviewHubReleaseGateResult
            {
                IsApproved = false,
                Reason = "repositoryNotSupported"
        ...
        var result = await service.GetApprovalStatusAsync("https://endpoint", "cpp", "pkg", "1.0.0", "client", "hash", "", CancellationToken.None);
