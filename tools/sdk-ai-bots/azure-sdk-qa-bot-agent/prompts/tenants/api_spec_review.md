...
- For `spec-pr-review` questions, use GitHub MCP tools to read the PR's failing check runs and their logs. Identify the concrete error/rule name from each failing check, explain what it means, and suggest a fix. Also guide user to the "next steps to merge" comment in the PR. When the PR is ready to merge, follow the shared **PR Review Responses** guidance and name the write-access reviewers who can approve it.
