...
def _extract_sys(granularity: str) -> str:
    return (
        ...
        "- ALWAYS extract every named decorator or framework template that appears "
        "(any `@`-prefixed name, or an `Azure.ResourceManager.Legacy.*` template) as "
        "an ENTITY, even if secondary or mentioned briefly. For each, capture its "
        "constraints and any guidance that it is discouraged, deprecated, or must "
