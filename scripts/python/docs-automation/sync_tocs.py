...
class PathResolver:
    def __init__(self, doc_repo_location = None, readme_suffix = ""):
    ...
    def amend_href(toc_dict):
        ...
        resolvable_path = os.path.join(self.doc_repo_location, input_string.replace("~/", ""))
...
def filter_toc(toc_dict, namespaces, repo_location, path_resolver):
    if toc_dict is None:
        return None
    ...
    if "items" in toc_dict:
        ...
        for item in toc_dict["items"]:
            result_n = filter_toc(item, namespaces, repo_location, path_resolver)
...
if __name__ == "__main__":
    ...
    path_resolver = new PathResolver(doc_repo_location=args.docrepo, readme_suffix=args.suffix)

    base_reference_toc[0] = filter_toc(base_reference_toc[0], present_in_target, args.docrepo, path_resolver)
