  ...
  */
  ...
  private checkFullApiReviewAndExecute() {
    ...
    if (!this.diffAPIRevision && this.hasReleasedApprovedGARevision) {
      this.confirmationService.confirm({
        message: 'You are requesting a Copilot review of the entire API surface. Did you intend to review only the changes? If so, switch to diff mode and then re-request.',
