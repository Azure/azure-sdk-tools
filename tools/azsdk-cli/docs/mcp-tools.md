...
| azsdk_update_release_plan_month | `azsdk release-plan update-month` | Update the SDK release target month on an existing release plan. Takes a work item ID and the target release month in `Month YYYY` format (e.g. `January 2026`). |
