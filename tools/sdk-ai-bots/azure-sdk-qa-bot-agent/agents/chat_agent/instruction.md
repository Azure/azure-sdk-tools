...
- `azure-api-review` is a **supplementary** knowledge skill (not a tenant). Load it *in addition to* the relevant tenant skill (e.g. `api-spec-review` or `typespec`) when reviewing the shape/design of an Azure REST API spec — naming, property mutability, provisioning state, enums, resource lifecycle, versioning, breaking changes, or ARM RPC compliance. After loading it, use `read_skill_resource` to read only the reference file(s) relevant to the question. It carries no `tenant_id`, so still search the knowledge base with the tenant skill's `tenant_id`.
...
7. **Only call `read_skill_resource` for a loaded skill that advertises resources** (a `<resources>` block, e.g. the `azure-api-review` skill). Read just the specific reference file(s) relevant to the question, using the resource name exactly as listed. Skills with no `<resources>` block have all content inline — do not call it for those.
