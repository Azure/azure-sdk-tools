...
namespace Azure.Sdk.Tools.Cli.Tools.GitHub
{
    ...
    ) : MCPMultiCommandTool
    {
        ...
        {
            var getForCurrentBranch = new Command(getPullRequestForCurrentBranchCommandName, "Get pull request for current branch");
            getForCurrentBranch.Options.Add(repoPathOpt);

            var createCommand = new Command(createPullRequestCommandName, "Create pull request");
            createCommand.Options.Add(titleOpt);
            createCommand.Options.Add(descriptionOpt);
            createCommand.Options.Add(repoPathOpt);
            createCommand.Options.Add(targetBranchOpt);
            createCommand.Options.Add(draftOpt);

            var getDetails = new Command(getPullRequestCommandName, "Get pull request details");
            getDetails.Options.Add(pullRequestNumberOpt);
            getDetails.Options.Add(repoPathOpt);

            return [getForCurrentBranch, createCommand, getDetails];
        }
