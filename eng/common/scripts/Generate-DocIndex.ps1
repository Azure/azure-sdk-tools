...
Param (
    $RepoRoot,
    $DocGenDir,
    $lang = "c", # Take c for example. should be empty string
    $packageNameRegex = "",
...
Do {
    ...
    if (!$pageToken) {
        ...
        # Next page call
        $resp = Invoke-RestMethod -Method Get -Uri "https://azuresdkdocs.blob.core.windows.net/%24web?restype=container&comp=list&prefix=$langRegex%2F&marker=$pageToken"
    }
    ...
    foreach ($elem in $xmlDoc.EnumerationResults.Blobs.BlobPrefix) {
        ...
        if ($blobPath -and ($blobPath -ne $artifact)) {
            # $blobRentionArray = $blobPath -split "," | ForEach-Object { $_.Trim() }   
            continue
        }
        ...
        $packagesInfo = $metadata | ? { $_.Package -eq $artifact -and ($_.GroupId -eq $defaultGroupId)}
        if ($packagesInfo.length -lt 1) {
            $packagesInfo = $metadata | ? { $_.Package -eq $artifact }
        }
        if (!$packagesInfo) {
            Write-Error "Did not find the artifacts from release csv. Please check and update."
        }
        elseif ($packagesInfo.length -gt 1) {
            Write-Warning "There are multiple artifacts fetching out for the same package name. "
            Write-Warning "Please check csv of Azure/azure-sdk/_data/release/latest repo if this is intended. "
        }
