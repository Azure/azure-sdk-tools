    ...
    public class CopilotPollingBackgroundHostedServiceTests
    {
        ...
        public async Task ProcessJobAsync_SuccessWithNoValidComments_StillMarksRevisionReviewed()
        {
            ...
            _mockApiRevisionsManager
                .Setup(x => x.UpdateAPIRevisionAsync(It.IsAny<APIRevisionListItemModel>()))
                .Callback<APIRevisionListItemModel>(rev => updatedRevision = rev)
            ...
            Assert.False(updatedRevision.CopilotReviewInProgress);
            Assert.True(updatedRevision.HasAutoGeneratedComments);
        }
